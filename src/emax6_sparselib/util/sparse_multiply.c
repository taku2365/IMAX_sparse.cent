...
int sparse_multiply_imax3(const int nnz,const emax6_sparse2* const A_sparse, const Uint* const B, Uint* C, int B_col_size,emax6_param* params){
    ...
    int B_col_blk = params->B_col_blk_param;
 ...
 int top,blk,h,w,blk_iter;
  ...
  Ull rofs,cofs;
  // AもBも縦方向に格納している
  for (top=0; top<B_col_size/NCHIP; top+=B_col_blk) { //B_col_blkごとに計算するBの列を動かす
...
/*3*/ for (CHIP=0; (CHIP<NCHIP)&&((A_margin[blk_iter]!=0)); CHIP++) { //marginが0の時は計算省略できる　marginはAの入れ替え時にみる
  /*2*/ for (rofs=0; rofs<A_margin[blk_iter]; rofs++) { //Aがどれだけrowを確保するか
    /*1*/ for (cofs=0; cofs<B_col_blk; cofs+=W) { // どれだけBをcolにすすめるか
            for (h=0; h<H; h+=2) { // 一回で2段を表している
                ...
                //*(float*)&C[(A_sort_index[rofs])*B_col_size(並べ替え後のCの行)+CHIP*B_col_size/NCHIP(CHIPごとの列)+top(B_col_blkごとのグループ)+cofs(wごとにB_col_blkが終わるまでcolに進む)+w(colに進む)]
                ...
                //*(float*)&B[A_val_index_set[h*A_row_size+2*rofs+blk*A_row_size+A_row_size*A_col_size]/4(実際のIMAXに合わせるために割る4 IMAXがAのアドレスをULLで読むのでsimdみたいに格納している)+1(simdの1)+(CHIP*B_col_size/NCHIP(CHIP分割)+top(B_col_blkごとに縦分割)+cofs(Bcol Wごとに更新)+w(Bcol 横幅))*B_row_size(かたまり全体でBのcol)]
...
int sparse_multiply_imax4(const int nnz,const emax6_sparse2* const A_sparse, const Uint* const B, Uint* C, int B_col_size,emax6_param* params){
 ...
 int top,blk,h,w,blk_iter;
  ...
  // AもBも縦方向に格納している
  for(top1=0;top1<A_row_size/NCHIP;top1+=B_col_blk1){ // CHIPごとにB_col_blkを分ける Cのため
    for (top=0; top<B_col_size; top+=B_col_blk) { //B_col_blkごとに計算するBの列を動かす
      for (rofs=0; rofs<B_col_blk1; rofs+=1) { //Aのrow
    ...
    /*2*/ for (rofs_blk=0; rofs_blk<A_margin[CHIP]/(A_row_size/NCHIP); rofs_blk++) { // Aのrow blkの飛び
      /*1*/ for (cofs=0; cofs<B_col_blk; cofs+=W) { // どれだけBをcolにすすめるか
              for (h=0; h<H; h+=2) { // 一回で2段を表している
                  ...
                  // *(float*)&A_val_index_set[h*A_margin[CHIP](unitごとのA 変形しているのでA_rowではなくA_margin)+rofs_blk*(A_row_size/NCHIP)(row_blkは構造的には縦だが、元の形は横にすすむblk)+(rofs+top1)(Aのrow CのB_col_blkの気にしながら)+A_margin_sum[CHIP]*H（CHIP分割）]
...
int sparse_multiply_imax5(const int nnz,const emax6_sparse2* const A_sparse, const Uint* const B, Uint* C, int B_col_size,emax6_param* params){
 ...
 int top,blk,h,w,blk_iter;
  ...
  for(top1=0;top1<A_row_size/NCHIP;top1+=B_col_blk1){ // CHIPごとにB_col_blkを分ける Cのため
    for (top=0; top<B_col_size; top+=B_col_blk) { //B_col_blkごとに計算するBの列を動かす
      for (cofs=0; cofs<B_col_blk; cofs+=W) { // どれだけBをcolにすすめるか
