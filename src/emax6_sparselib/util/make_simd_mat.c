...
static void make_simd_random_mat_pad(emax6_param* emax6_param,Uint* B,Uint* B_debug){
  ...
  for (col=0,col1=0; col<(B_col_size+B_col_pad)/2; col+=1,col1+=2){
    for (row=0,row1=0; row1<(B_row_size+B_H_pad); row+=2,row1+=1) {
       ...
       printf("B in\n");
      #endif
      *(float*)&B[col*2*(B_row_size+B_H_pad)+row] = *(float*)&B_debug[col1*(B_row_size+B_H_pad)+row1];
      *(float*)&B[col*2*(B_row_size+B_H_pad)+row+1] = *(float*)&B_debug[(col1+1)*(B_row_size+B_H_pad)+row1];
    }
  }
