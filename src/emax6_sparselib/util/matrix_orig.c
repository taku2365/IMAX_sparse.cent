  ...
  for (row=0; row<A_row_size; row++) {
    for (col=0; col<B_col_size; col++) {
      for (n=0; n<(A_col_size+A_H_pad); n++) {
        if (n==0) *(float*)&C_orig[row+col*A_row_size]  = *(float*)&A_orig[row+n*A_row_size] * *(float*)&B_orig[n+col*(B_row_size+A_H_pad)];
        else      *(float*)&C_orig[row+col*A_row_size] += *(float*)&A_orig[row+n*A_row_size] * *(float*)&B_orig[n+col*(B_row_size+A_H_pad)];
        count0++;
        /*printf("[%d %d %d]", row, col, n);*/
      }
      /*printf("\n");*/
