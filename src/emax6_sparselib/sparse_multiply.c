    ...
    /*1*/ for (int cofs=0; cofs<RMGRP; cofs+=W) { // どれだけBをcolにすすめるか
            for (int w=0; w<W; w++) {   /* horizontal (parallel) execution */
              for (int h=0; h<H; h++) { /* vertical (pipelined) execution */
                ...
                //*(float*)&B[*((Uint*)&A_val_index_set[h*A_row_size+rofs+blk*A_row_size]+1)(前段のunitから伝搬するBrow(Acol) 前段から伝播するので1+がない)+(CHIP*B_col_size/NCHIP(CHIP分割)+top(RMGRPごとに縦分割)+cofs(Bcol Wごとに更新)+w(Bcol 横幅))*B_row_size(Bのcol)]
                // C = Aの行×Bの列
                if (blk == 0 && h == 0)
                  *(float*)&C[(A_sort_index[rofs])*B_col_size+CHIP*B_col_size/NCHIP+top+cofs+w]  = *(float*)&A_val_index_set[1+h*A_row_size+rofs+blk*A_row_size]**(float*)&B[*((Uint*)&A_val_index_set[h*A_row_size+rofs+blk*A_row_size]+1)+(CHIP*B_col_size/NCHIP+top+cofs+w)*B_row_size];
                else
                  *(float*)&C[(A_sort_index[rofs])*B_col_size+CHIP*B_col_size/NCHIP+top+cofs+w] += *(float*)&A_val_index_set[1+h*A_row_size+rofs+blk*A_row_size]**(float*)&B[*((Uint*)&A_val_index_set[h*A_row_size+rofs+blk*A_row_size]+1)+(CHIP*B_col_size/NCHIP+top+cofs+w)*B_row_size];
