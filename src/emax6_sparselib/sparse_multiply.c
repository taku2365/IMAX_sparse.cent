...
/*3*/ for (CHIP=0; (CHIP<NCHIP)&&((A_margin[blk_iter]!=0)); CHIP++) { //marginが0の時は計算省略できる　marginはAの入れ替え時にみる
  /*2*/ for (rofs=0; rofs<A_margin[blk_iter]; rofs++) { //Aがどれだけrowを確保するか
    /*1*/ for (cofs=0; cofs<RMGRP; cofs+=W) { // どれだけBをcolにすすめるか
            for (h=0; h<H; h+=2) { /* vertical (pipelined) execution */
                for (w=0; w<W; w+=2) {    // Bcol +=2
                count++;
                // AもBもCも縦方向に格納している。
                // A  32bit val : 32bit next unit Brow
                ...
                //+(CHIP*B_col_size/NCHIP+top+cofs+w)*B_row_sizeでsimdのために2をかけないのはw+=2ですでに実現しているから。
                  ...
                  *(float*)&C[(A_sort_index[rofs])/4+1+(CHIP*B_col_size/NCHIP+top+cofs+w)*B_row_size]  += *(float*)&A_val_index_set[(h+1)*A_row_size+rofs+blk*A_row_size]**(float*)&B[A_val_index_set[h*A_row_size+2*rofs+blk*A_row_size+A_row_size*A_col_size+1]/4+1+(CHIP*B_col_size/NCHIP+top+cofs+w)*B_row_size];
