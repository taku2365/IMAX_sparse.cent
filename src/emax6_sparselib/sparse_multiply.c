...
void sparse_multiply_imax1(const emax6_sparse* const A_sparse, const Uint* const B, Uint* C, int B_col_size,int NCHIP,int RMGRP){
    ...
    for(int top=0; top<A_row_size/NCHIP; top+=RMGRP){
        for(int i=0; i<RMGRP;i++){
            for(int CHIP=0; CHIP<NCHIP; CHIP++){
                for(int j=0; j<B_col_size;j++){
                    for(int k=A_row_p[i]; k<A_row_p[i+1]; k++){
                        if (k==0) *(float*)&C[(CHIP*A_row_size/NCHIP+i+top)*B_col_size+j]  = *(float*)&A_nnz_val[k] * *(float*)&B[A_nnz_col_index[k]*B_col_size+j];
                        else *(float*)&C[(CHIP*A_row_size/NCHIP+i+top)*B_col_size+j]  += *(float*)&A_nnz_val[k] * *(float*)&B[A_nnz_col_index[k]*B_col_size+j];
                    }
                }
            }
        }
