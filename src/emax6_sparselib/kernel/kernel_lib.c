...
/*3*/ for (CHIP=0; CHIP<NCHIP; CHIP++) { /* will be parallelized by multi-chip (M/#chip) */
  ...
  /*2*/    for (INIT0=1,LOOP0=A_margin_tmp,rofs=(0-1*8)<<32|((0-1*4)&0xffffffff); LOOP0--; INIT0=0) {  /* stage#0 *//* mapped to FOR() on BR[63][1][0] */
            ...
            exe(OP_ADD,    &rofs, INIT0?rofs:rofs, EXP_H3210, (1*8)<<32|((1*4)&0xffffffff), EXP_H3210, 0LL, EXP_H3210, OP_AND, 0xffffffffffffffffLL, OP_NOP, 0LL);/* stage#0 */
...
/*3*/ for (CHIP=0; CHIP<NCHIP; CHIP++) { /* will be parallelized by multi-chip (M/#chip) */
  ...
  /*2*/    for (INIT0=1,LOOP0=A_margin_tmp,rofs=(0-1*8)<<32|((0-1*4)&0xffffffff); LOOP0--; INIT0=0) {  /* stage#0 *//* mapped to FOR() on BR[63][1][0] */
            exe(OP_ADD,    &rofs, INIT0?rofs:rofs, EXP_H3210, 1*8<<32|1*4, EXP_H3210, 0LL, EXP_H3210, OP_AND, 0xffffffffffffffffLL, OP_NOP, 0LL);/* stage#0 */ //1*8LL<<32|1*4LLとしてはいけない!!!!
            ...
            exe(OP_ADD,    &c_index1, (Ull)c_index, EXP_H3210, 0LL, EXP_H3210, 0LL, EXP_H3210, OP_AND, 0xffffffff, OP_NOP, 0LL);            /* stage#1 */
...
void sparse_gemm_736_736_736_CHIP_div_B_5(Uint* C, const Uint* A, const Uint* B, emax6_sparse2* A_sparse) {
  // args: A IMAX_CSR_format
  // arg B fortran continues simd format
  // arg C fortran continues simd format
  // A_col_blk=2 スケジューリング方法を変更
