...
void sparse_gemm_CHIP_div_B_impl3(Uint* C, const Uint* A, const Uint* B, emax6_sparse2* A_sparse, emax6_param* params) {
    ...
    Uint top,blk,B_col_blk_tmp,C_top_tmp,C_col_blk_tmp;
    Ull x,y,z,t;
    ...
    size_t C_col_blk = params->C_col_blk_param;
    /*size_t NCHIP 4*/
    ...
    size_t C_col_blk_mul_A_row_size = C_col_blk*A_row_size;
    ...
    for (blk=0,blk_iter=0,A_col_blk_tmp=A_col_blk,A_row_size_mul_2_mul_A_col_blk = A_row_size*2*A_col_blk; blk<A_col_size; blk+=A_col_blk*H,blk_iter+=A_col_blk) { /* 3重ループ展開の外側対象 */
        ...
        C_top_tmp = 0;
        C_col_blk_tmp = 0; 
        for (B_col_blk_tmp=0; B_col_blk_tmp<B_col_blk; B_col_blk_tmp+=W*2,C_col_blk_tmp+=W*2){
            if((B_col_blk_tmp%C_col_blk == 0)&&(B_col_blk != C_col_blk)){
                C_top_tmp = B_col_blk_tmp;
                C_col_blk_tmp = 0;
            }
            for (CHIP=0; CHIP<NCHIP; CHIP++) { 
                ...
                c0[CHIP] = C+(CHIP*B_col_size/NCHIP+C_top_tmp+top)*A_row_size;
    ...
    #define sparse_final1(ar,ar_pre,br,br_pre,index_val) \
	    mop(OP_LDR,  3, &BR[br][0][1],  (Ull)c00[CHIP],(Ull)index_val, MSK_W0, (Ull)c0[CHIP], C_col_blk_mul_A_row_size, 0, 1, (Ull)NULL, C_col_blk_mul_A_row_size);\
	    mop(OP_LDR,  3, &BR[br][1][1],  (Ull)c01[CHIP],(Ull)index_val, MSK_W0, (Ull)c0[CHIP], C_col_blk_mul_A_row_size, 0, 1, (Ull)NULL, C_col_blk_mul_A_row_size);\
	    mop(OP_LDR,  3, &BR[br][2][1],  (Ull)c02[CHIP],(Ull)index_val, MSK_W0, (Ull)c0[CHIP], C_col_blk_mul_A_row_size, 0, 1, (Ull)NULL, C_col_blk_mul_A_row_size);\
	    mop(OP_LDR,  3, &BR[br][3][1],  (Ull)c03[CHIP],(Ull)index_val, MSK_W0, (Ull)c0[CHIP], C_col_blk_mul_A_row_size, 0, 1, (Ull)NULL, C_col_blk_mul_A_row_size);\
	    ...
	    mop(OP_STR, 3, &AR[ar][0], (Ull)index_val, (Ull)c00[CHIP],  MSK_D0, (Ull)c0[CHIP], C_col_blk_mul_A_row_size, 0, 1, (Ull)NULL, C_col_blk_mul_A_row_size);\
	    mop(OP_STR, 3, &AR[ar][1], (Ull)index_val, (Ull)c01[CHIP],  MSK_D0, (Ull)c0[CHIP], C_col_blk_mul_A_row_size, 0, 1, (Ull)NULL, C_col_blk_mul_A_row_size);\
	    mop(OP_STR, 3, &AR[ar][2], (Ull)index_val, (Ull)c02[CHIP],  MSK_D0, (Ull)c0[CHIP], C_col_blk_mul_A_row_size, 0, 1, (Ull)NULL, C_col_blk_mul_A_row_size);\
	    mop(OP_STR, 3, &AR[ar][3], (Ull)index_val, (Ull)c03[CHIP],  MSK_D0, (Ull)c0[CHIP], C_col_blk_mul_A_row_size, 0, 1, (Ull)NULL, C_col_blk_mul_A_row_size)
