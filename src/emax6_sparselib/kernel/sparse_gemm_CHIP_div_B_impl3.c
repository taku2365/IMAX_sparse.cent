...
void sparse_gemm_CHIP_div_B_impl3(Uint* C, const Uint* A, const Uint* B, emax6_sparse2* A_sparse, emax6_param* params) {
  ...
  Uint top,top_c,top_c_tmp,blk,B_col_blk_tmp = 0,C_col_blk_tmp = 0,C_col_blk_tmp1 = 0;
  Ull x,y,z,t;
  ...
  for (top=0; top<B_col_size/NCHIP; top+=B_col_blk) {
    for (blk=0,blk_iter=0,A_col_blk_tmp=A_col_blk,A_row_size_mul_2_mul_A_col_blk = A_row_size*2*A_col_blk; blk<A_col_size; blk+=A_col_blk*H,blk_iter+=A_col_blk) { /* 3重ループ展開の外側対象 */
      ...
      C_col_blk_tmp = 0;
      for (B_col_blk_tmp=0; B_col_blk_tmp<B_col_blk; B_col_blk_tmp+=W*2){
        ...
        if(((top_c+C_col_blk) == (top+B_col_blk_tmp))&&(top_c != B_col_size)){
            top_c += C_col_blk;
            C_col_blk_tmp = 0;
        } 
        for (CHIP=0; CHIP<NCHIP; CHIP++) { 
          ...
          b0[CHIP] = (Uint*)b[CHIP]+B_col_blk_tmp*B_row_size+0; b1[CHIP] = (Uint*)b[CHIP]+B_col_blk_tmp*B_row_size+B_row_size*2; b2[CHIP] = (Uint*)b[CHIP]+B_col_blk_tmp*B_row_size+B_row_size*4;  b3[CHIP] = (Uint*)b[CHIP]+B_col_blk_tmp*B_row_size+B_row_size*6; 


          c0[CHIP] = C+(CHIP*B_col_size/NCHIP+top_c)*A_row_size;
          c00[CHIP]= (Uint*)c0[CHIP]+C_col_blk_tmp*A_row_size+0; c01[CHIP] = (Uint*)c0[CHIP]+C_col_blk_tmp*A_row_size+A_row_size*2; c02[CHIP] = (Uint*)c0[CHIP]+C_col_blk_tmp*A_row_size+A_row_size*4; c03[CHIP] = (Uint*)c0[CHIP]+C_col_blk_tmp*A_row_size+A_row_size*6;
  ...
  #define sparse_core1_1(ar,ar_pre,br,br_pre,a_index,a_prev,a_current,a_next) \
      exe(OP_FMA, &AR[ar][0], AR[ar_pre][0], EXP_H3210, a_prev , EXP_H1010, BR[br_pre][0][1], EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);\
	    ...
	    mop(OP_LDR,  3, &BR[br][0][1],  (Ull)b0[CHIP],(Ull)a_current, MSK_W1, (Ull)b[CHIP],B_row_size_mul_B_col_blk, 0, 0, (Ull)NULL, B_row_size_mul_B_col_blk);\
	    mop(OP_LDR,  3, &BR[br][0][0],  (Ull)b1[CHIP],(Ull)a_current, MSK_W1, (Ull)b[CHIP],B_row_size_mul_B_col_blk, 0, 0, (Ull)NULL, B_row_size_mul_B_col_blk);\
	    mop(OP_LDR,  3, &BR[br][1][1],  (Ull)b2[CHIP],(Ull)a_current, MSK_W1, (Ull)b[CHIP],B_row_size_mul_B_col_blk, 0, 0, (Ull)NULL, B_row_size_mul_B_col_blk);\
	    mop(OP_LDR,  3, &BR[br][1][0],  (Ull)b3[CHIP],(Ull)a_current, MSK_W1, (Ull)b[CHIP],B_row_size_mul_B_col_blk, 0, 0, (Ull)NULL, B_row_size_mul_B_col_blk);\
...
#define sparse_final1(ar,ar_pre,br,br_pre,index_val) \
	    mop(OP_LDR,  3, &BR[br][0][1],  (Ull)c00[CHIP],(Ull)index_val, MSK_W0, (Ull)c0[CHIP], A_row_size_mul_C_col_blk, 0, 1, (Ull)NULL, A_row_size_mul_C_col_blk);\
	    mop(OP_LDR,  3, &BR[br][1][1],  (Ull)c01[CHIP],(Ull)index_val, MSK_W0, (Ull)c0[CHIP], A_row_size_mul_C_col_blk, 0, 1, (Ull)NULL, A_row_size_mul_C_col_blk);\
	    mop(OP_LDR,  3, &BR[br][2][1],  (Ull)c02[CHIP],(Ull)index_val, MSK_W0, (Ull)c0[CHIP], A_row_size_mul_C_col_blk, 0, 1, (Ull)NULL, A_row_size_mul_C_col_blk);\
	    mop(OP_LDR,  3, &BR[br][3][1],  (Ull)c03[CHIP],(Ull)index_val, MSK_W0, (Ull)c0[CHIP], A_row_size_mul_C_col_blk, 0, 1, (Ull)NULL, A_row_size_mul_C_col_blk);\
	    ...
	    mop(OP_STR, 3, &AR[ar][0], (Ull)index_val, (Ull)c00[CHIP],  MSK_D0, (Ull)c0[CHIP], A_row_size_mul_C_col_blk, 0, 1, (Ull)NULL, A_row_size_mul_C_col_blk);\
	    mop(OP_STR, 3, &AR[ar][1], (Ull)index_val, (Ull)c01[CHIP],  MSK_D0, (Ull)c0[CHIP], A_row_size_mul_C_col_blk, 0, 1, (Ull)NULL, A_row_size_mul_C_col_blk);\
	    mop(OP_STR, 3, &AR[ar][2], (Ull)index_val, (Ull)c02[CHIP],  MSK_D0, (Ull)c0[CHIP], A_row_size_mul_C_col_blk, 0, 1, (Ull)NULL, A_row_size_mul_C_col_blk);\
	    mop(OP_STR, 3, &AR[ar][3], (Ull)index_val, (Ull)c03[CHIP],  MSK_D0, (Ull)c0[CHIP], A_row_size_mul_C_col_blk, 0, 1, (Ull)NULL, A_row_size_mul_C_col_blk)


//EMAX5A begin mm3 mapdist=0
/*3*/ for (CHIP=0; CHIP<NCHIP; CHIP++) { /* will be parallelized by multi-chip (M/#chip) */
  ...
  /*2*/    for (INIT0=1,LOOP0=A_margin_tmp,rofs=rofs_init; LOOP0--; INIT0=0) {  /* stage#0 *//* mapped to FOR() on BR[63][1][0] */
            ...
            mop(OP_LDR,  3, &BR[3][0][1], (Ull)b0[CHIP], a0, MSK_W1, (Ull)b[CHIP],B_row_size_mul_B_col_blk, 0, 0, (Ull)NULL,B_row_size_mul_B_col_blk);             /* stage#2 */
            mop(OP_LDR,  3, &BR[3][0][0], (Ull)b1[CHIP], a0, MSK_W1, (Ull)b[CHIP],B_row_size_mul_B_col_blk, 0, 0, (Ull)NULL,B_row_size_mul_B_col_blk);             /* stage#2 */
            mop(OP_LDR,  3, &BR[3][1][1], (Ull)b2[CHIP], a0, MSK_W1, (Ull)b[CHIP],B_row_size_mul_B_col_blk, 0, 0, (Ull)NULL,B_row_size_mul_B_col_blk);             /* stage#2 */
            mop(OP_LDR,  3, &BR[3][1][0], (Ull)b3[CHIP], a0, MSK_W1, (Ull)b[CHIP],B_row_size_mul_B_col_blk, 0, 0, (Ull)NULL,B_row_size_mul_B_col_blk);             /* stage#2 2KB */
            ...
            mop(OP_LDR,  3, &BR[4][0][1],  (Ull)b0[CHIP],(Ull)a1, MSK_W1, (Ull)b[CHIP],B_row_size_mul_B_col_blk, 0, 0, (Ull)NULL, B_row_size_mul_B_col_blk);
            mop(OP_LDR,  3, &BR[4][0][0],  (Ull)b1[CHIP],(Ull)a1, MSK_W1, (Ull)b[CHIP],B_row_size_mul_B_col_blk, 0, 0, (Ull)NULL, B_row_size_mul_B_col_blk);
            mop(OP_LDR,  3, &BR[4][1][1],  (Ull)b2[CHIP],(Ull)a1, MSK_W1, (Ull)b[CHIP],B_row_size_mul_B_col_blk, 0, 0, (Ull)NULL, B_row_size_mul_B_col_blk);
            mop(OP_LDR,  3, &BR[4][1][0],  (Ull)b3[CHIP],(Ull)a1, MSK_W1, (Ull)b[CHIP],B_row_size_mul_B_col_blk, 0, 0, (Ull)NULL, B_row_size_mul_B_col_blk);
