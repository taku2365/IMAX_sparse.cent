  ...
  char **argv;
{
  ...
  if (argc != 3) {
    printf("usage: %s K <file>     ... read text\n", *argv);
    printf("usage: %s K <file>.bin ... read bin (generate if non-existent)\n", *argv);
    exit(1);
  }
  ...
  sysinit(sizeof(struct vertex)*MAXVERTICES
	  ...
	  sizeof(struct neighborvertex)*MAXNV_PPAGE);
...
#if 0
  if (!strncmp(argv[2]+(((strlen(argv[1])-4)>0)? strlen(argv[2])-4 : 0), ".bin", 4)) {
    if ((fd = _open(argv[2], O_RDONLY)) > 0) { /* found */
	...
	printf("can't reuse binary_file %s (my.vhashtbl=%08.8x file.vhashtbl=%08.8x)\n",
	       argv[2], vhashtbl, vhashtbl_fileaddr);
	_close(fd);
	*(argv[2]+strlen(argv[2])-4) = 0;
	...
	goto skip_regedge;
      ...
      if ((fd = _open(argv[2], O_CREAT | O_TRUNC | O_WRONLY, 0644)) < 0) { /* create failed */
  ...
  for (each_K=1; each_K<=num_K; each_K++) {
	...
	vcount++;
	if (maxdist < vertex[vindex].Kdistance)
	  maxdist = vertex[vindex].Kdistance;
      }
    }
