...
/-EMAX5AS-/       /*3*/ for (CHIP=0; CHIP<NCHIP; CHIP++) { /* output channels are parallelized by multi-chip (OC4/#chip) */
/-EMAX5AS-/         /*2*/ for (INIT1=1,LOOP1=""BATCH"",img=""(0-IMX4)<<32|((0-M4)&0xffffffff)""; LOOP1--; INIT1=0) {                           /* mapped to FOR() on BR[63][1][0] */ /* stage#0 */
/-EMAX5AS-/           /*1*/ for (INIT0=1,LOOP0=""M"",cofs=""(0-4LL)<<32|((0-4LL)&0xffffffff)""; LOOP0--; INIT0=0) {                            /* mapped to FOR() on BR[63][0][0] */ /* stage#0 */
/-EMAX5AS-/                   exe(OP_ADD,      &img,  img,             EXP_H3210,  INIT0?IMX4M4:0, EXP_H3210,  0LL, EXP_H3210, OP_NOP,   0LL,                  OP_NOP, 0LL); /* stage#0 */
/-EMAX5AS-/ 		  exe(OP_ADD,      &cofs, INIT0?cofs:cofs, EXP_H3210,  4LL<<32|4LL,    EXP_H3210,  0LL, EXP_H3210, OP_AND,   0xffffffffffffffffLL, OP_NOP, 0LL); /* stage#0 */
/-EMAX5AS-/ 		  exe(OP_ADD,      &iofs, img,             EXP_H3210,  cofs,           EXP_H3210,  0LL, EXP_H3210, OP_AND,   0xffffffff00000000LL, OP_NOP, 0LL); /* stage#1 */
/-EMAX5AS-/ 		  exe(OP_ADD,      &oofs, img,             EXP_H3210,  cofs,           EXP_H3210,  0LL, EXP_H3210, OP_AND,   0x00000000ffffffffLL, OP_NOP, 0LL); /* stage#1 */
#if 1
/-EMAX5AS-/ 		  back_g_ker_core1( 2,  0,  0); /**** oc0 ic0*****/
/-EMAX5AS-/ 		  back_g_ker_core1( 3,  0,  1); /**** oc0 ic1*****/
/-EMAX5AS-/ 		  back_g_ker_core1( 4,  0,  2); /**** oc0 ic1*****/
/-EMAX5AS-/ 		  back_g_ker_core1( 5,  0,  3); /**** oc0 ic1*****/
#endif
/-EMAX5AS-/                 }
/-EMAX5AS-/               }
/-EMAX5AS-/             }
