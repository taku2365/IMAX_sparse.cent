...
{
  int i, j, k;
  sysinit(256*256*sizeof(float)
         +(256 +8)*256*sizeof(float)
         +(256 +256)*sizeof(Uint)
         +256*256*sizeof(float)
         +256*256*sizeof(float)
         +256*256*sizeof(float)
         +256*256*sizeof(float)
         +256*256*sizeof(float),32);
  printf("membase: %08.8x\n", (Uint)membase);
  A0 = (float*)membase;
  A = (float*)((Uchar*)A0 + 256*256*sizeof(float));
  p = (Uint*) ((Uchar*)A +(256 +8)*256*sizeof(float));
  inv0= (float*)((Uchar*)p +(256 +256)*sizeof(Uint));
  inv1= (float*)((Uchar*)inv0+ 256*256*sizeof(float));
  b = (float*)((Uchar*)inv1+ 256*256*sizeof(float));
  x = (float*)((Uchar*)b + 256*256*sizeof(float));
  C = (float*)((Uchar*)x + 256*256*sizeof(float));
  printf("A0  : %08.8x\n", A0);
  printf("A   : %08.8x\n", A);
  printf("p   : %08.8x\n", p);
  printf("inv0: %08.8x\n", inv0);
  ...
  for (i=0; i<256; i++) {
    for (j=0; j<256; j++)
      A[i*256 +j] = A0[i*256 +j] = (float)(i%256 +j);
  }
  A[0] = A0[0] = 1;
  for (j=1;j<256;j++)
    A[j*256 +j] = A0[j*256 +j] = 3;
  x11_open(0);
  reset_nanosec();
  imax();
  get_nanosec(0);
  ...
  for (i=0; i<256; i++) {
    for (j=0; j<256; j++) {
      for (k=0; k<256; k++) {
        if (k==0) C[i*256 +j] = A0[i*256 +k] * inv1[k*256 +j];
        else C[i*256 +j] += A0[i*256 +k] * inv1[k*256 +j];
      }
      if (i == j && fabsf(C[i*256 +j]-1.0)>(5.0E-3)) {
 count2++;
 printf("A*A'!=E C[%d][%d]=%f\n", i, j, C[i*256 +j]);
      }
      else if (i != j && (fabsf(C[i*256 +j])>(5.0E-3))) {
 count2++;
 printf("A*A'!=E C[%d][%d]=%f\n", i, j, C[i*256 +j]);
      }
    }
     ...
     unsigned int *from;
{
  ...
  case 4: offs = from + 256*HT; break;
  case 5: offs = from + 256*HT+WD; break;
  case 6: offs = from + 256*HT+WD*2; break;
  case 7: offs = from + 256*HT+WD*3; break;
  case 8: offs = from + 256*HT*2; break;
  case 9: offs = from + 256*HT*2+WD; break;
  case 10: offs = from + 256*HT*2+WD*2; break;
  case 11: offs = from + 256*HT*2+WD*3; break;
  case 12: offs = from + 256*HT*3; break;
  case 13: offs = from + 256*HT*3+WD; break;
  case 14: offs = from + 256*HT*3+WD*2; break;
  case 15: offs = from + 256*HT*3+WD*3; break;
  }
  for (i=0; i<HT; i++, offs+=256) {
    if (offs<from+256*256) {
      for (j=0; j<WD; j++) {
 if (j+(id%4)*WD<256) *to++ = (*(offs+j))>>0;
 else *to++ = 0;
      }
    }
    else {
...
orig()
{
  ...
  for (i=0; i<256; i++) {
    ...
    k = -1;
    for (j=i; j<256; j++) {
      if (pmax < fabsf(A[p[j]*256 +i])) {
 pmax = fabsf(A[p[j]*256 +i]);
 k = j;
      }
    }
    if (k == -1) {
    ...
    j = p[k]; p[k] = p[i]; p[i] = j;
    A[p[i]*256 +i] = 1.0/A[p[i]*256 +i];
    for (j=i+1; j<256; j++) {
      A[p[j]*256 +i] *= A[p[i]*256 +i];
      for (k=i+1; k<256; k++)
 A[p[j]*256 +k] -= A[p[j]*256 +i]*A[p[i]*256 +k];
    }
  }
  for (i=0; i<256; i++) {
    for (j=0; j<256; j++)
    ...
    for (j=i+1; j<256; j++) {
      for (k=i; k<j; k++)
 b[p[j]] -= A[p[j]*256 +k]*b[p[k]];
    }
    for (j=256 -1; j>=0; j--) {
      for (k=256 -1; k>j; k--)
 b[p[j]] -= A[p[j]*256 +k]*x[k];
      inv0[j*256 +p[i]] = x[j] = b[p[j]]*A[p[j]*256 +j];
    }
  }
}
imax() {
  ...
  for (i=0; i<256; i++) {
    ...
    k = -1;
    for (j=i; j<256; j++) {
      if (pmax < fabsf(A[j*256 +i])) {
 pmax = fabsf(A[j*256 +i]);
 k = j;
      }
    }
    if (k == -1) {
    ...
    j = p[k]; p[k] = p[i]; p[i] = j;
    for (j=0; j<256; j++) {
      tmp = A[k*256 +j]; A[k*256 +j] = A[i*256 +j]; A[i*256 +j] = tmp;
    }
    A[i*256 +i] = 1.0/A[i*256 +i];
    for (j=i+1; j<256; j++)
      A[j*256 +i] *= A[i*256 +i];
    Uint *top = &A[i*256 +i];
    Uint *topw = (Ull)top;
    Uint len = 256 -i;
    Uint len2 = len+(8 -1)*256;
    Uint grp;
    if (len < 16) {
      for (j=i+1; j<256; j+=1*16*8) {
 for (CHIP=0; CHIP<1; CHIP++) {
   for (h=0; h<16; h++) {
     for (grp=0; grp<8; grp++) {
       for (k=0; k<256 -(i+1); k++) {
  if (j+h*1*8 +CHIP*8 +grp<256) A[(j+h*1*8 +CHIP*8 +grp)*256 +i+1+k] -= A[(j+h*1*8 +CHIP*8 +grp)*256 +i]*A[i*256 +i+1+k];
       }
     }
   }
 }
    ...
    for (j=i+1; j<256; j+=1*16*8) {
      Uint l00[1], l01[1], l02[1], l03[1], l04[1], l05[1], l06[1], l07[1];
      Uint l08[1], l09[1], l10[1], l11[1], l12[1], l13[1], l14[1], l15[1];
      Uint *d00[1], *d01[1], *d02[1], *d03[1], *d04[1], *d05[1], *d06[1], *d07[1];
      Uint *d08[1], *d09[1], *d10[1], *d11[1], *d12[1], *d13[1], *d14[1], *d15[1];
      Uint *d00w[1],*d01w[1],*d02w[1],*d03w[1],*d04w[1],*d05w[1],*d06w[1],*d07w[1];
      Uint *d08w[1],*d09w[1],*d10w[1],*d11w[1],*d12w[1],*d13w[1],*d14w[1],*d15w[1];
      for (CHIP=0; CHIP<1; CHIP++) {
 l00[CHIP]=(j+ 0*1*8 +CHIP*8<256)?(j+ 0*1*8 +CHIP*8):256;l01[CHIP]=(j+ 1*1*8 +CHIP*8<256)?(j+ 1*1*8 +CHIP*8):256;
        l02[CHIP]=(j+ 2*1*8 +CHIP*8<256)?(j+ 2*1*8 +CHIP*8):256;l03[CHIP]=(j+ 3*1*8 +CHIP*8<256)?(j+ 3*1*8 +CHIP*8):256;
 l04[CHIP]=(j+ 4*1*8 +CHIP*8<256)?(j+ 4*1*8 +CHIP*8):256;l05[CHIP]=(j+ 5*1*8 +CHIP*8<256)?(j+ 5*1*8 +CHIP*8):256;
        l06[CHIP]=(j+ 6*1*8 +CHIP*8<256)?(j+ 6*1*8 +CHIP*8):256;l07[CHIP]=(j+ 7*1*8 +CHIP*8<256)?(j+ 7*1*8 +CHIP*8):256;
 l08[CHIP]=(j+ 8*1*8 +CHIP*8<256)?(j+ 8*1*8 +CHIP*8):256;l09[CHIP]=(j+ 9*1*8 +CHIP*8<256)?(j+ 9*1*8 +CHIP*8):256;
        l10[CHIP]=(j+10*1*8 +CHIP*8<256)?(j+10*1*8 +CHIP*8):256;l11[CHIP]=(j+11*1*8 +CHIP*8<256)?(j+11*1*8 +CHIP*8):256;
 l12[CHIP]=(j+12*1*8 +CHIP*8<256)?(j+12*1*8 +CHIP*8):256;l13[CHIP]=(j+13*1*8 +CHIP*8<256)?(j+13*1*8 +CHIP*8):256;
        l14[CHIP]=(j+14*1*8 +CHIP*8<256)?(j+14*1*8 +CHIP*8):256;l15[CHIP]=(j+15*1*8 +CHIP*8<256)?(j+15*1*8 +CHIP*8):256;
 d00[CHIP] = &A[l00[CHIP]*256 +i]; d01[CHIP] = &A[l01[CHIP]*256 +i]; d02[CHIP] = &A[l02[CHIP]*256 +i]; d03[CHIP] = &A[l03[CHIP]*256 +i];
 d04[CHIP] = &A[l04[CHIP]*256 +i]; d05[CHIP] = &A[l05[CHIP]*256 +i]; d06[CHIP] = &A[l06[CHIP]*256 +i]; d07[CHIP] = &A[l07[CHIP]*256 +i];
 d08[CHIP] = &A[l08[CHIP]*256 +i]; d09[CHIP] = &A[l09[CHIP]*256 +i]; d10[CHIP] = &A[l10[CHIP]*256 +i]; d11[CHIP] = &A[l11[CHIP]*256 +i];
 d12[CHIP] = &A[l12[CHIP]*256 +i]; d13[CHIP] = &A[l13[CHIP]*256 +i]; d14[CHIP] = &A[l14[CHIP]*256 +i]; d15[CHIP] = &A[l15[CHIP]*256 +i];
...
#ifdef EMAXSC
/* EMAXSC start */
SCBR[0].r[0][0][0] = l00[0];
SCBR[0].r[0][1][0] = l00[0];
SCBR[0].r[0][0][1] = 256;
SCBR[0].r[0][1][1] = 256;
SCBR[1].r[0][0][0] = BR[2][0][1];
SCBR[1].r[0][1][0] = BR[2][0][1];
SCBR[1].r[0][0][1] = BR[2][1][1];
SCBR[1].r[0][1][1] = BR[2][1][1];
SCBR[1].r[0][0][3] = BR[2][2][1];
SCBR[1].r[0][1][3] = BR[2][2][1];
SCBR[1].r[0][0][4] = 0;
SCBR[1].r[0][1][4] = 0;
SCBR[1].r[0][0][5] = l01[0];
SCBR[1].r[0][1][5] = l01[0];
SCBR[1].r[0][0][7] = 256;
SCBR[1].r[0][1][7] = 256;
SCBR[2].r[0][0][0] = BR[3][0][1];
SCBR[2].r[0][1][0] = BR[3][0][1];
SCBR[2].r[0][0][1] = BR[3][1][1];
SCBR[2].r[0][1][1] = BR[3][1][1];
SCBR[2].r[0][0][3] = BR[3][2][1];
SCBR[2].r[0][1][3] = BR[3][2][1];
SCBR[2].r[0][0][4] = 0;
SCBR[2].r[0][1][4] = 0;
SCBR[2].r[0][0][7] = l02[0];
SCBR[2].r[0][1][7] = l02[0];
SCBR[2].r[0][0][8] = 256;
SCBR[2].r[0][1][8] = 256;
SCBR[3].r[0][0][0] = BR[4][0][1];
SCBR[3].r[0][1][0] = BR[4][0][1];
SCBR[3].r[0][0][1] = BR[4][1][1];
SCBR[3].r[0][1][1] = BR[4][1][1];
SCBR[3].r[0][0][3] = BR[4][2][1];
SCBR[3].r[0][1][3] = BR[4][2][1];
SCBR[3].r[0][0][4] = 0;
SCBR[3].r[0][1][4] = 0;
SCBR[3].r[0][0][7] = l03[0];
SCBR[3].r[0][1][7] = l03[0];
SCBR[3].r[0][0][8] = 256;
SCBR[3].r[0][1][8] = 256;
SCBR[4].r[0][0][0] = BR[5][0][1];
SCBR[4].r[0][1][0] = BR[5][0][1];
SCBR[4].r[0][0][1] = BR[5][1][1];
SCBR[4].r[0][1][1] = BR[5][1][1];
SCBR[4].r[0][0][3] = BR[5][2][1];
SCBR[4].r[0][1][3] = BR[5][2][1];
SCBR[4].r[0][0][4] = 0;
SCBR[4].r[0][1][4] = 0;
SCBR[4].r[0][0][7] = l04[0];
SCBR[4].r[0][1][7] = l04[0];
SCBR[4].r[0][0][8] = 256;
SCBR[4].r[0][1][8] = 256;
SCBR[5].r[0][0][0] = BR[6][0][1];
SCBR[5].r[0][1][0] = BR[6][0][1];
SCBR[5].r[0][0][1] = BR[6][1][1];
SCBR[5].r[0][1][1] = BR[6][1][1];
SCBR[5].r[0][0][3] = BR[6][2][1];
SCBR[5].r[0][1][3] = BR[6][2][1];
SCBR[5].r[0][0][4] = 0;
SCBR[5].r[0][1][4] = 0;
SCBR[5].r[0][0][7] = l05[0];
SCBR[5].r[0][1][7] = l05[0];
SCBR[5].r[0][0][8] = 256;
SCBR[5].r[0][1][8] = 256;
SCBR[6].r[0][0][0] = BR[7][0][1];
SCBR[6].r[0][1][0] = BR[7][0][1];
SCBR[6].r[0][0][1] = BR[7][1][1];
SCBR[6].r[0][1][1] = BR[7][1][1];
SCBR[6].r[0][0][3] = BR[7][2][1];
SCBR[6].r[0][1][3] = BR[7][2][1];
SCBR[6].r[0][0][4] = 0;
SCBR[6].r[0][1][4] = 0;
SCBR[6].r[0][0][7] = l06[0];
SCBR[6].r[0][1][7] = l06[0];
SCBR[6].r[0][0][8] = 256;
SCBR[6].r[0][1][8] = 256;
SCBR[7].r[0][0][0] = BR[8][0][1];
SCBR[7].r[0][1][0] = BR[8][0][1];
SCBR[7].r[0][0][1] = BR[8][1][1];
SCBR[7].r[0][1][1] = BR[8][1][1];
SCBR[7].r[0][0][3] = BR[8][2][1];
SCBR[7].r[0][1][3] = BR[8][2][1];
SCBR[7].r[0][0][4] = 0;
SCBR[7].r[0][1][4] = 0;
SCBR[7].r[0][0][7] = l07[0];
SCBR[7].r[0][1][7] = l07[0];
SCBR[7].r[0][0][8] = 256;
SCBR[7].r[0][1][8] = 256;
SCBR[8].r[0][0][0] = BR[9][0][1];
SCBR[8].r[0][1][0] = BR[9][0][1];
SCBR[8].r[0][0][1] = BR[9][1][1];
SCBR[8].r[0][1][1] = BR[9][1][1];
SCBR[8].r[0][0][3] = BR[9][2][1];
SCBR[8].r[0][1][3] = BR[9][2][1];
SCBR[8].r[0][0][4] = 0;
SCBR[8].r[0][1][4] = 0;
SCBR[8].r[0][0][7] = l08[0];
SCBR[8].r[0][1][7] = l08[0];
SCBR[8].r[0][0][8] = 256;
SCBR[8].r[0][1][8] = 256;
SCBR[9].r[0][0][0] = BR[10][0][1];
SCBR[9].r[0][1][0] = BR[10][0][1];
SCBR[9].r[0][0][1] = BR[10][1][1];
SCBR[9].r[0][1][1] = BR[10][1][1];
SCBR[9].r[0][0][3] = BR[10][2][1];
SCBR[9].r[0][1][3] = BR[10][2][1];
SCBR[9].r[0][0][4] = 0;
SCBR[9].r[0][1][4] = 0;
SCBR[9].r[0][0][7] = l09[0];
SCBR[9].r[0][1][7] = l09[0];
SCBR[9].r[0][0][8] = 256;
SCBR[9].r[0][1][8] = 256;
SCBR[10].r[0][0][0] = BR[11][0][1];
SCBR[10].r[0][1][0] = BR[11][0][1];
SCBR[10].r[0][0][1] = BR[11][1][1];
SCBR[10].r[0][1][1] = BR[11][1][1];
SCBR[10].r[0][0][3] = BR[11][2][1];
SCBR[10].r[0][1][3] = BR[11][2][1];
SCBR[10].r[0][0][4] = 0;
SCBR[10].r[0][1][4] = 0;
SCBR[10].r[0][0][7] = l10[0];
SCBR[10].r[0][1][7] = l10[0];
SCBR[10].r[0][0][8] = 256;
SCBR[10].r[0][1][8] = 256;
SCBR[11].r[0][0][0] = BR[12][0][1];
SCBR[11].r[0][1][0] = BR[12][0][1];
SCBR[11].r[0][0][1] = BR[12][1][1];
SCBR[11].r[0][1][1] = BR[12][1][1];
SCBR[11].r[0][0][3] = BR[12][2][1];
SCBR[11].r[0][1][3] = BR[12][2][1];
SCBR[11].r[0][0][4] = 0;
SCBR[11].r[0][1][4] = 0;
SCBR[11].r[0][0][7] = l11[0];
SCBR[11].r[0][1][7] = l11[0];
SCBR[11].r[0][0][8] = 256;
SCBR[11].r[0][1][8] = 256;
SCBR[12].r[0][0][0] = BR[13][0][1];
SCBR[12].r[0][1][0] = BR[13][0][1];
SCBR[12].r[0][0][1] = BR[13][1][1];
SCBR[12].r[0][1][1] = BR[13][1][1];
SCBR[12].r[0][0][3] = BR[13][2][1];
SCBR[12].r[0][1][3] = BR[13][2][1];
SCBR[12].r[0][0][4] = 0;
SCBR[12].r[0][1][4] = 0;
SCBR[12].r[0][0][7] = l12[0];
SCBR[12].r[0][1][7] = l12[0];
SCBR[12].r[0][0][8] = 256;
SCBR[12].r[0][1][8] = 256;
SCBR[13].r[0][0][0] = BR[14][0][1];
SCBR[13].r[0][1][0] = BR[14][0][1];
SCBR[13].r[0][0][1] = BR[14][1][1];
SCBR[13].r[0][1][1] = BR[14][1][1];
SCBR[13].r[0][0][3] = BR[14][2][1];
SCBR[13].r[0][1][3] = BR[14][2][1];
SCBR[13].r[0][0][4] = 0;
SCBR[13].r[0][1][4] = 0;
SCBR[13].r[0][0][7] = l13[0];
SCBR[13].r[0][1][7] = l13[0];
SCBR[13].r[0][0][8] = 256;
SCBR[13].r[0][1][8] = 256;
SCBR[14].r[0][0][0] = BR[15][0][1];
SCBR[14].r[0][1][0] = BR[15][0][1];
SCBR[14].r[0][0][1] = BR[15][1][1];
SCBR[14].r[0][1][1] = BR[15][1][1];
SCBR[14].r[0][0][3] = BR[15][2][1];
SCBR[14].r[0][1][3] = BR[15][2][1];
SCBR[14].r[0][0][4] = 0;
SCBR[14].r[0][1][4] = 0;
SCBR[14].r[0][0][7] = l14[0];
SCBR[14].r[0][1][7] = l14[0];
SCBR[14].r[0][0][8] = 256;
SCBR[14].r[0][1][8] = 256;
SCBR[15].r[0][0][0] = BR[16][0][1];
SCBR[15].r[0][1][0] = BR[16][0][1];
SCBR[15].r[0][0][1] = BR[16][1][1];
SCBR[15].r[0][1][1] = BR[16][1][1];
SCBR[15].r[0][0][3] = BR[16][2][1];
SCBR[15].r[0][1][3] = BR[16][2][1];
SCBR[15].r[0][0][4] = 0;
SCBR[15].r[0][1][4] = 0;
SCBR[15].r[0][0][7] = l15[0];
SCBR[15].r[0][1][7] = l15[0];
SCBR[15].r[0][0][8] = 256;
SCBR[15].r[0][1][8] = 256;
SCBR[16].r[0][0][0] = BR[17][0][1];
SCBR[16].r[0][1][0] = BR[17][0][1];
SCBR[16].r[0][0][1] = BR[17][1][1];
SCBR[16].r[0][1][1] = BR[17][1][1];
SCBR[16].r[0][0][3] = BR[17][2][1];
SCBR[16].r[0][1][3] = BR[17][2][1];
SCBR[16].r[0][0][4] = 0;
SCBR[16].r[0][1][4] = 0;
...
SCBR[63].r[0][0][6] = (256)*(4);
SCBR[63].r[0][1][6] = (256)*(4);
SCM0[2].o[0][0] = (Ull)d00[0];
SCM0[3].o[0][0] = (Ull)d01[0];
SCM0[4].o[0][0] = (Ull)d02[0];
SCM0[5].o[0][0] = (Ull)d03[0];
SCM0[6].o[0][0] = (Ull)d04[0];
SCM0[7].o[0][0] = (Ull)d05[0];
SCM0[8].o[0][0] = (Ull)d06[0];
SCM0[9].o[0][0] = (Ull)d07[0];
SCM0[10].o[0][0] = (Ull)d08[0];
SCM0[11].o[0][0] = (Ull)d09[0];
SCM0[12].o[0][0] = (Ull)d10[0];
SCM0[13].o[0][0] = (Ull)d11[0];
SCM0[14].o[0][0] = (Ull)d12[0];
SCM0[15].o[0][0] = (Ull)d13[0];
SCM0[16].o[0][0] = (Ull)d14[0];
SCM0[17].o[0][0] = (Ull)d15[0];
sc_param[0].LOOP0=LOOP0; sc_param[0].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[0], 0, emax6sc_pth_inv_x1_00, &sc_param[0]);
sc_param[1].LOOP0=LOOP0; sc_param[1].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[1], 0, emax6sc_pth_inv_x1_01, &sc_param[1]);
sc_param[2].LOOP0=LOOP0; sc_param[2].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[2], 0, emax6sc_pth_inv_x1_02, &sc_param[2]);
sc_param[3].LOOP0=LOOP0; sc_param[3].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[3], 0, emax6sc_pth_inv_x1_03, &sc_param[3]);
sc_param[4].LOOP0=LOOP0; sc_param[4].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[4], 0, emax6sc_pth_inv_x1_04, &sc_param[4]);
sc_param[5].LOOP0=LOOP0; sc_param[5].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[5], 0, emax6sc_pth_inv_x1_05, &sc_param[5]);
sc_param[6].LOOP0=LOOP0; sc_param[6].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[6], 0, emax6sc_pth_inv_x1_06, &sc_param[6]);
sc_param[7].LOOP0=LOOP0; sc_param[7].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[7], 0, emax6sc_pth_inv_x1_07, &sc_param[7]);
sc_param[8].LOOP0=LOOP0; sc_param[8].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[8], 0, emax6sc_pth_inv_x1_08, &sc_param[8]);
sc_param[9].LOOP0=LOOP0; sc_param[9].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[9], 0, emax6sc_pth_inv_x1_09, &sc_param[9]);
sc_param[10].LOOP0=LOOP0; sc_param[10].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[10], 0, emax6sc_pth_inv_x1_10, &sc_param[10]);
sc_param[11].LOOP0=LOOP0; sc_param[11].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[11], 0, emax6sc_pth_inv_x1_11, &sc_param[11]);
sc_param[12].LOOP0=LOOP0; sc_param[12].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[12], 0, emax6sc_pth_inv_x1_12, &sc_param[12]);
sc_param[13].LOOP0=LOOP0; sc_param[13].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[13], 0, emax6sc_pth_inv_x1_13, &sc_param[13]);
sc_param[14].LOOP0=LOOP0; sc_param[14].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[14], 0, emax6sc_pth_inv_x1_14, &sc_param[14]);
sc_param[15].LOOP0=LOOP0; sc_param[15].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[15], 0, emax6sc_pth_inv_x1_15, &sc_param[15]);
sc_param[16].LOOP0=LOOP0; sc_param[16].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[16], 0, emax6sc_pth_inv_x1_16, &sc_param[16]);
sc_param[17].LOOP0=LOOP0; sc_param[17].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[17], 0, emax6sc_pth_inv_x1_17, &sc_param[17]);
...
/* EMAXSC end */
	...
	emax6.mapdist = 0;
	*(Uint*)&emax6.lmmi[0][2][0][emax6.lmmic] = ((len2-1)<<16)|0x1007|(1<<2);
	emax6.lmmi[0][2][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][2][0][emax6.lmmic].top = d00w[0];
	*(Uint*)&emax6.lmmi[0][3][0][emax6.lmmic] = ((len2-1)<<16)|0x1007|(1<<2);
	emax6.lmmi[0][3][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][3][0][emax6.lmmic].top = d01w[0];
	*(Uint*)&emax6.lmmi[0][4][0][emax6.lmmic] = ((len2-1)<<16)|0x1007|(1<<2);
	emax6.lmmi[0][4][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][4][0][emax6.lmmic].top = d02w[0];
	*(Uint*)&emax6.lmmi[0][5][0][emax6.lmmic] = ((len2-1)<<16)|0x1007|(1<<2);
	emax6.lmmi[0][5][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][5][0][emax6.lmmic].top = d03w[0];
	*(Uint*)&emax6.lmmi[0][6][0][emax6.lmmic] = ((len2-1)<<16)|0x1007|(1<<2);
	emax6.lmmi[0][6][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][6][0][emax6.lmmic].top = d04w[0];
	*(Uint*)&emax6.lmmi[0][7][0][emax6.lmmic] = ((len2-1)<<16)|0x1007|(1<<2);
	emax6.lmmi[0][7][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][7][0][emax6.lmmic].top = d05w[0];
	*(Uint*)&emax6.lmmi[0][8][0][emax6.lmmic] = ((len2-1)<<16)|0x1007|(1<<2);
	emax6.lmmi[0][8][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][8][0][emax6.lmmic].top = d06w[0];
	*(Uint*)&emax6.lmmi[0][9][0][emax6.lmmic] = ((len2-1)<<16)|0x1007|(1<<2);
	emax6.lmmi[0][9][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][9][0][emax6.lmmic].top = d07w[0];
	*(Uint*)&emax6.lmmi[0][10][0][emax6.lmmic] = ((len2-1)<<16)|0x1007|(1<<2);
	emax6.lmmi[0][10][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][10][0][emax6.lmmic].top = d08w[0];
	*(Uint*)&emax6.lmmi[0][11][0][emax6.lmmic] = ((len2-1)<<16)|0x1007|(1<<2);
	emax6.lmmi[0][11][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][11][0][emax6.lmmic].top = d09w[0];
	*(Uint*)&emax6.lmmi[0][12][0][emax6.lmmic] = ((len2-1)<<16)|0x1007|(1<<2);
	emax6.lmmi[0][12][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][12][0][emax6.lmmic].top = d10w[0];
	*(Uint*)&emax6.lmmi[0][13][0][emax6.lmmic] = ((len2-1)<<16)|0x1007|(1<<2);
	emax6.lmmi[0][13][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][13][0][emax6.lmmic].top = d11w[0];
	*(Uint*)&emax6.lmmi[0][14][0][emax6.lmmic] = ((len2-1)<<16)|0x1007|(1<<2);
	emax6.lmmi[0][14][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][14][0][emax6.lmmic].top = d12w[0];
	*(Uint*)&emax6.lmmi[0][15][0][emax6.lmmic] = ((len2-1)<<16)|0x1007|(1<<2);
	emax6.lmmi[0][15][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][15][0][emax6.lmmic].top = d13w[0];
	*(Uint*)&emax6.lmmi[0][16][0][emax6.lmmic] = ((len2-1)<<16)|0x1007|(1<<2);
	emax6.lmmi[0][16][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][16][0][emax6.lmmic].top = d14w[0];
	*(Uint*)&emax6.lmmi[0][17][0][emax6.lmmic] = ((len2-1)<<16)|0x1007|(1<<2);
	emax6.lmmi[0][17][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][17][0][emax6.lmmic].top = d15w[0];
	emax6.lmmi_bitmap[0] = 0x000000000003fffcLL;
	...
	if (emax6.last_conf == emax6_conf_inv_x1) {
	  ...
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 6, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 7, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 8, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 9, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 10, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 11, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 12, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 13, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 14, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 15, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 16, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 17, 0);/*drain*/
	}
	...
	if (emax6.last_conf != emax6_conf_inv_x1) {
	  ...
	  emax6.last_conf = emax6_conf_inv_x1;
	  emax6.lastdist = 0;
	  dst = (Dll*)(((struct reg_ctrl*)emax6.reg_ctrl)->i[0].conf);
	  src = (Dll*)emax6_conf_inv_x1;
	...
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[0][0].br[0] = l00[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[0][0].br[1] = 256;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[1][0].br[0] = BR[2][0][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[1][0].br[1] = BR[2][1][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[1][0].br[3] = BR[2][2][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[1][1].br[0] = 0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[1][1].br[1] = l01[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[1][1].br[3] = 256;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[2][0].br[0] = BR[3][0][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[2][0].br[1] = BR[3][1][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[2][0].br[3] = BR[3][2][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[2][1].br[0] = 0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[2][1].br[3] = l02[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[2][2].br[0] = 256;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[3][0].br[0] = BR[4][0][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[3][0].br[1] = BR[4][1][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[3][0].br[3] = BR[4][2][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[3][1].br[0] = 0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[3][1].br[3] = l03[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[3][2].br[0] = 256;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[4][0].br[0] = BR[5][0][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[4][0].br[1] = BR[5][1][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[4][0].br[3] = BR[5][2][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[4][1].br[0] = 0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[4][1].br[3] = l04[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[4][2].br[0] = 256;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[5][0].br[0] = BR[6][0][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[5][0].br[1] = BR[6][1][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[5][0].br[3] = BR[6][2][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[5][1].br[0] = 0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[5][1].br[3] = l05[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[5][2].br[0] = 256;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[6][0].br[0] = BR[7][0][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[6][0].br[1] = BR[7][1][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[6][0].br[3] = BR[7][2][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[6][1].br[0] = 0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[6][1].br[3] = l06[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[6][2].br[0] = 256;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[7][0].br[0] = BR[8][0][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[7][0].br[1] = BR[8][1][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[7][0].br[3] = BR[8][2][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[7][1].br[0] = 0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[7][1].br[3] = l07[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[7][2].br[0] = 256;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[8][0].br[0] = BR[9][0][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[8][0].br[1] = BR[9][1][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[8][0].br[3] = BR[9][2][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[8][1].br[0] = 0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[8][1].br[3] = l08[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[8][2].br[0] = 256;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[9][0].br[0] = BR[10][0][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[9][0].br[1] = BR[10][1][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[9][0].br[3] = BR[10][2][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[9][1].br[0] = 0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[9][1].br[3] = l09[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[9][2].br[0] = 256;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[10][0].br[0] = BR[11][0][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[10][0].br[1] = BR[11][1][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[10][0].br[3] = BR[11][2][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[10][1].br[0] = 0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[10][1].br[3] = l10[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[10][2].br[0] = 256;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[11][0].br[0] = BR[12][0][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[11][0].br[1] = BR[12][1][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[11][0].br[3] = BR[12][2][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[11][1].br[0] = 0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[11][1].br[3] = l11[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[11][2].br[0] = 256;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[12][0].br[0] = BR[13][0][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[12][0].br[1] = BR[13][1][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[12][0].br[3] = BR[13][2][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[12][1].br[0] = 0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[12][1].br[3] = l12[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[12][2].br[0] = 256;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[13][0].br[0] = BR[14][0][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[13][0].br[1] = BR[14][1][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[13][0].br[3] = BR[14][2][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[13][1].br[0] = 0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[13][1].br[3] = l13[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[13][2].br[0] = 256;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[14][0].br[0] = BR[15][0][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[14][0].br[1] = BR[15][1][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[14][0].br[3] = BR[15][2][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[14][1].br[0] = 0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[14][1].br[3] = l14[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[14][2].br[0] = 256;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[15][0].br[0] = BR[16][0][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[15][0].br[1] = BR[16][1][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[15][0].br[3] = BR[16][2][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[15][1].br[0] = 0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[15][1].br[3] = l15[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[15][2].br[0] = 256;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[16][0].br[0] = BR[17][0][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[16][0].br[1] = BR[17][1][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[16][0].br[3] = BR[17][2][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[16][1].br[0] = 0;
	...
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[63][1].br[0] = 4LL;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[63][1].br[1] = rofs;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[63][1].br[2] = (256)*(4);
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[2][0].ea0o = (Ull)d00[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[3][0].ea0o = (Ull)d01[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[4][0].ea0o = (Ull)d02[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[5][0].ea0o = (Ull)d03[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[6][0].ea0o = (Ull)d04[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[7][0].ea0o = (Ull)d05[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[8][0].ea0o = (Ull)d06[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[9][0].ea0o = (Ull)d07[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[10][0].ea0o = (Ull)d08[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[11][0].ea0o = (Ull)d09[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[12][0].ea0o = (Ull)d10[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[13][0].ea0o = (Ull)d11[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[14][0].ea0o = (Ull)d12[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[15][0].ea0o = (Ull)d13[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[16][0].ea0o = (Ull)d14[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[17][0].ea0o = (Ull)d15[0];
	...
	*(Ull*)&(reg_ctrl->i[0].addr[17][0].top) = ((Ull)(emax6.lmmi[0][17][0][lmmic].top+*((Ushort*)&emax6.lmmi[0][17][0][lmmic]+1)*sizeof(Uint)+(sizeof(Uint)-1))<<32) | (Ull)(Uint)emax6.lmmi[0][17][0][lmmic].top;
	...
	emax6.lmmd[6][0] = 0xff>>7;
	emax6.lmmd[7][0] = 0xff>>7;
	emax6.lmmd[8][0] = 0xff>>7;
	emax6.lmmd[9][0] = 0xff>>7;
	emax6.lmmd[10][0] = 0xff>>7;
	emax6.lmmd[11][0] = 0xff>>7;
	emax6.lmmd[12][0] = 0xff>>7;
	emax6.lmmd[13][0] = 0xff>>7;
	emax6.lmmd[14][0] = 0xff>>7;
	emax6.lmmd[15][0] = 0xff>>7;
	emax6.lmmd[16][0] = 0xff>>7;
	emax6.lmmd[17][0] = 0xff>>7;
	while (((struct reg_ctrl*)emax6.reg_ctrl)->i[0].stat); //LMRING_BUSY|EXRING_BUSY
	get_nanosec(NANOS_EXEC);
asm volatile("b emax6_conf_end_inv_x1\n"
".align 5\n"
".global emax6_conf_inv_x1\n"
"emax6_conf_inv_x1:\n"
...
"	.word	0x00000000, 0x00000000\n"
"	.word	0x00000000, 0x00000000\n"
"	.word	0x30200023, 0x00000000\n"
"	.word	0xaaaa4446, 0x02000e52\n"
"	.word	0xab000000, 0x00068000\n"
"	.word	0x00000000, 0x00000000\n"
"	.word	0x00e0a035, 0x00000000\n"
"	.word	0xffff0000, 0x00000000\n"
...
"	.word	0x00000000, 0x00000000\n"
"	.word	0x30200023, 0x00000000\n"
"	.word	0xaaaa4446, 0x05000e52\n"
"	.word	0xab000000, 0x00068000\n"
"	.word	0x00000000, 0x00000000\n"
"	.word	0x0100e035, 0x00000000\n"
"	.word	0xffff0000, 0x00000000\n"
"	.word	0x00080050, 0x00000070\n"
"	.word	0x00000000, 0x00000000\n"
"	.word	0x00000001, 0x00000000\n"
"	.word	0xffff0000, 0x00000000\n"
"	.word	0x00000000, 0x00000000\n"
...
"	.word	0x00000000, 0x00000000\n"
"	.word	0x00000000, 0x00000000\n"
".global emax6_conf_end_inv_x1\n"
"emax6_conf_end_inv_x1:\n"
);
#endif
    }
#ifndef EMAXSC
  ...
  for (i=0; i<256; i++) {
    for (j=0; j<256; j++)
      b[i*256 +j] = (i==j)?1.0:0.0;
  }
  for (i=0; i<256; i+=1*16) {
    Uint l000[1], l010[1], l020[1], l030[1], l040[1], l050[1], l060[1], l070[1];
    Uint l080[1], l090[1], l100[1], l110[1], l120[1], l130[1], l140[1], l150[1];
    Uint *t000[1], *t010[1], *t020[1], *t030[1], *t040[1], *t050[1], *t060[1], *t070[1];
    Uint *t080[1], *t090[1], *t100[1], *t110[1], *t120[1], *t130[1], *t140[1], *t150[1];
    Uint *t000w[1],*t010w[1],*t020w[1],*t030w[1],*t040w[1],*t050w[1],*t060w[1],*t070w[1];
    Uint *t080w[1],*t090w[1],*t100w[1],*t110w[1],*t120w[1],*t130w[1],*t140w[1],*t150w[1];
    for (CHIP=0; CHIP<1; CHIP++) {
      l000[CHIP] = (i+CHIP*16 + 0+0)*256; l010[CHIP] = (i+CHIP*16 + 1+0)*256; l020[CHIP] = (i+CHIP*16 + 2+0)*256; l030[CHIP] = (i+CHIP*16 + 3+0)*256;
      l040[CHIP] = (i+CHIP*16 + 4+0)*256; l050[CHIP] = (i+CHIP*16 + 5+0)*256; l060[CHIP] = (i+CHIP*16 + 6+0)*256; l070[CHIP] = (i+CHIP*16 + 7+0)*256;
      l080[CHIP] = (i+CHIP*16 + 8+0)*256; l090[CHIP] = (i+CHIP*16 + 9+0)*256; l100[CHIP] = (i+CHIP*16 +10+0)*256; l110[CHIP] = (i+CHIP*16 +11+0)*256;
      l120[CHIP] = (i+CHIP*16 +12+0)*256; l130[CHIP] = (i+CHIP*16 +13+0)*256; l140[CHIP] = (i+CHIP*16 +14+0)*256; l150[CHIP] = (i+CHIP*16 +15+0)*256;
 ...
 for (CHIP=0; CHIP<1; CHIP++) {
   for (h=0; h<16; h++) {
     for (k=i; k<j; k++) {
       b[(i+CHIP*16 +h)*256 +j] -= A[j*256 +k]*b[(i+CHIP*16 +h)*256 +k];
     }
   }
 }
      }
      ...
      Ull Ajk;
      Ull b000, b001;
      Uint *d000[1], *d010[1], *d020[1], *d030[1], *d040[1], *d050[1], *d060[1], *d070[1];
      Uint *d080[1], *d090[1], *d100[1], *d110[1], *d120[1], *d130[1], *d140[1], *d150[1];
      Uint *d000w[1],*d010w[1],*d020w[1],*d030w[1],*d040w[1],*d050w[1],*d060w[1],*d070w[1];
      Uint *d080w[1],*d090w[1],*d100w[1],*d110w[1],*d120w[1],*d130w[1],*d140w[1],*d150w[1];
      for (CHIP=0; CHIP<1; CHIP++) {
...
SCBR[0].r[0][0][0] = Ajk;
SCBR[0].r[0][1][0] = Ajk;
SCBR[0].r[0][0][1] = BR[2][3][1];
SCBR[0].r[0][1][1] = BR[2][3][1];
SCBR[1].r[0][0][0] = Ajk;
SCBR[1].r[0][1][0] = Ajk;
SCBR[1].r[0][0][1] = BR[3][3][1];
SCBR[1].r[0][1][1] = BR[3][3][1];
SCBR[2].r[0][0][0] = Ajk;
SCBR[2].r[0][1][0] = Ajk;
SCBR[2].r[0][0][1] = BR[4][3][1];
SCBR[2].r[0][1][1] = BR[4][3][1];
SCBR[3].r[0][0][0] = Ajk;
SCBR[3].r[0][1][0] = Ajk;
SCBR[3].r[0][0][1] = BR[5][3][1];
SCBR[3].r[0][1][1] = BR[5][3][1];
SCBR[4].r[0][0][0] = Ajk;
SCBR[4].r[0][1][0] = Ajk;
SCBR[4].r[0][0][1] = BR[6][3][1];
SCBR[4].r[0][1][1] = BR[6][3][1];
SCBR[5].r[0][0][0] = Ajk;
SCBR[5].r[0][1][0] = Ajk;
SCBR[5].r[0][0][1] = BR[7][3][1];
SCBR[5].r[0][1][1] = BR[7][3][1];
SCBR[6].r[0][0][0] = Ajk;
SCBR[6].r[0][1][0] = Ajk;
SCBR[6].r[0][0][1] = BR[8][3][1];
SCBR[6].r[0][1][1] = BR[8][3][1];
SCBR[7].r[0][0][0] = Ajk;
SCBR[7].r[0][1][0] = Ajk;
SCBR[7].r[0][0][1] = BR[9][3][1];
SCBR[7].r[0][1][1] = BR[9][3][1];
SCBR[8].r[0][0][0] = Ajk;
SCBR[8].r[0][1][0] = Ajk;
SCBR[8].r[0][0][1] = BR[10][3][1];
SCBR[8].r[0][1][1] = BR[10][3][1];
SCBR[9].r[0][0][0] = Ajk;
SCBR[9].r[0][1][0] = Ajk;
SCBR[9].r[0][0][1] = BR[11][3][1];
SCBR[9].r[0][1][1] = BR[11][3][1];
SCBR[10].r[0][0][0] = Ajk;
SCBR[10].r[0][1][0] = Ajk;
SCBR[10].r[0][0][1] = BR[12][3][1];
SCBR[10].r[0][1][1] = BR[12][3][1];
SCBR[11].r[0][0][0] = Ajk;
SCBR[11].r[0][1][0] = Ajk;
SCBR[11].r[0][0][1] = BR[13][3][1];
SCBR[11].r[0][1][1] = BR[13][3][1];
SCBR[12].r[0][0][0] = Ajk;
SCBR[12].r[0][1][0] = Ajk;
SCBR[12].r[0][0][1] = BR[14][3][1];
SCBR[12].r[0][1][1] = BR[14][3][1];
SCBR[13].r[0][0][0] = Ajk;
SCBR[13].r[0][1][0] = Ajk;
SCBR[13].r[0][0][1] = BR[15][3][1];
SCBR[13].r[0][1][1] = BR[15][3][1];
SCBR[14].r[0][0][0] = Ajk;
SCBR[14].r[0][1][0] = Ajk;
SCBR[14].r[0][0][1] = BR[16][3][1];
SCBR[14].r[0][1][1] = BR[16][3][1];
SCBR[63].r[0][0][0] = LOOP0;
SCBR[63].r[0][1][0] = LOOP0;
SCBR[63].r[0][0][1] = -1LL;
SCBR[63].r[0][1][1] = -1LL;
SCBR[63].r[0][0][2] = cofs;
SCBR[63].r[0][1][2] = cofs;
SCBR[63].r[0][0][3] = 4LL;
SCBR[63].r[0][1][3] = 4LL;
SCBR[63].r[0][0][4] = b000;
SCBR[63].r[0][1][4] = b000;
SCBR[63].r[0][0][5] = Ajk;
SCBR[63].r[0][1][5] = Ajk;
SCBR[63].r[0][0][6] = BR[1][3][1];
SCBR[63].r[0][1][6] = BR[1][3][1];
SCM0[0].b[0][2] = (Ull)0;
SCM0[0].o[0][2] = (Ull)d000[0];
SCM0[1].b[0][0] = (Ull)0;
SCM0[1].o[0][0] = (Ull)d010[0];
SCM0[2].b[0][0] = (Ull)0;
SCM0[2].o[0][0] = (Ull)d020[0];
SCM0[3].b[0][0] = (Ull)0;
SCM0[3].o[0][0] = (Ull)d030[0];
SCM0[4].b[0][0] = (Ull)0;
SCM0[4].o[0][0] = (Ull)d040[0];
SCM0[5].b[0][0] = (Ull)0;
SCM0[5].o[0][0] = (Ull)d050[0];
SCM0[6].b[0][0] = (Ull)0;
SCM0[6].o[0][0] = (Ull)d060[0];
SCM0[7].b[0][0] = (Ull)0;
SCM0[7].o[0][0] = (Ull)d070[0];
SCM0[8].b[0][0] = (Ull)0;
SCM0[8].o[0][0] = (Ull)d080[0];
SCM0[9].b[0][0] = (Ull)0;
SCM0[9].o[0][0] = (Ull)d090[0];
SCM0[10].b[0][0] = (Ull)0;
SCM0[10].o[0][0] = (Ull)d100[0];
SCM0[11].b[0][0] = (Ull)0;
SCM0[11].o[0][0] = (Ull)d110[0];
SCM0[12].b[0][0] = (Ull)0;
SCM0[12].o[0][0] = (Ull)d120[0];
SCM0[13].b[0][0] = (Ull)0;
SCM0[13].o[0][0] = (Ull)d130[0];
SCM0[14].b[0][0] = (Ull)0;
SCM0[14].o[0][0] = (Ull)d140[0];
SCM0[15].b[0][0] = (Ull)0;
SCM0[15].o[0][0] = (Ull)d150[0];
sc_param[0].LOOP0=LOOP0; sc_param[0].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[0], 0, emax6sc_pth_inv_x2_00, &sc_param[0]);
sc_param[1].LOOP0=LOOP0; sc_param[1].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[1], 0, emax6sc_pth_inv_x2_01, &sc_param[1]);
sc_param[2].LOOP0=LOOP0; sc_param[2].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[2], 0, emax6sc_pth_inv_x2_02, &sc_param[2]);
sc_param[3].LOOP0=LOOP0; sc_param[3].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[3], 0, emax6sc_pth_inv_x2_03, &sc_param[3]);
sc_param[4].LOOP0=LOOP0; sc_param[4].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[4], 0, emax6sc_pth_inv_x2_04, &sc_param[4]);
sc_param[5].LOOP0=LOOP0; sc_param[5].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[5], 0, emax6sc_pth_inv_x2_05, &sc_param[5]);
sc_param[6].LOOP0=LOOP0; sc_param[6].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[6], 0, emax6sc_pth_inv_x2_06, &sc_param[6]);
sc_param[7].LOOP0=LOOP0; sc_param[7].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[7], 0, emax6sc_pth_inv_x2_07, &sc_param[7]);
sc_param[8].LOOP0=LOOP0; sc_param[8].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[8], 0, emax6sc_pth_inv_x2_08, &sc_param[8]);
sc_param[9].LOOP0=LOOP0; sc_param[9].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[9], 0, emax6sc_pth_inv_x2_09, &sc_param[9]);
sc_param[10].LOOP0=LOOP0; sc_param[10].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[10], 0, emax6sc_pth_inv_x2_10, &sc_param[10]);
sc_param[11].LOOP0=LOOP0; sc_param[11].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[11], 0, emax6sc_pth_inv_x2_11, &sc_param[11]);
sc_param[12].LOOP0=LOOP0; sc_param[12].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[12], 0, emax6sc_pth_inv_x2_12, &sc_param[12]);
sc_param[13].LOOP0=LOOP0; sc_param[13].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[13], 0, emax6sc_pth_inv_x2_13, &sc_param[13]);
sc_param[14].LOOP0=LOOP0; sc_param[14].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[14], 0, emax6sc_pth_inv_x2_14, &sc_param[14]);
sc_param[15].LOOP0=LOOP0; sc_param[15].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[15], 0, emax6sc_pth_inv_x2_15, &sc_param[15]);
pthread_join(*(pthread_t*)&sc_pth[0], 0);
pthread_join(*(pthread_t*)&sc_pth[1], 0);
pthread_join(*(pthread_t*)&sc_pth[2], 0);
pthread_join(*(pthread_t*)&sc_pth[3], 0);
...
/* EMAXSC end */
	...
	emax6.lmmic = 1-emax6.lmmic;
	emax6.mapdist = 0;
	*(Uint*)&emax6.lmmi[0][0][2][emax6.lmmic] = 0x00001007|(1<<2);
	emax6.lmmi[0][0][2][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][0][2][emax6.lmmic].top = d000w[0];
	*(Uint*)&emax6.lmmi[0][1][0][emax6.lmmic] = 0x00001007|(1<<2);
	emax6.lmmi[0][1][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][1][0][emax6.lmmic].top = d010w[0];
	*(Uint*)&emax6.lmmi[0][2][0][emax6.lmmic] = 0x00001007|(1<<2);
	emax6.lmmi[0][2][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][2][0][emax6.lmmic].top = d020w[0];
	*(Uint*)&emax6.lmmi[0][3][0][emax6.lmmic] = 0x00001007|(1<<2);
	emax6.lmmi[0][3][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][3][0][emax6.lmmic].top = d030w[0];
	*(Uint*)&emax6.lmmi[0][4][0][emax6.lmmic] = 0x00001007|(1<<2);
	emax6.lmmi[0][4][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][4][0][emax6.lmmic].top = d040w[0];
	*(Uint*)&emax6.lmmi[0][5][0][emax6.lmmic] = 0x00001007|(1<<2);
	emax6.lmmi[0][5][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][5][0][emax6.lmmic].top = d050w[0];
	*(Uint*)&emax6.lmmi[0][6][0][emax6.lmmic] = 0x00001007|(1<<2);
	emax6.lmmi[0][6][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][6][0][emax6.lmmic].top = d060w[0];
	*(Uint*)&emax6.lmmi[0][7][0][emax6.lmmic] = 0x00001007|(1<<2);
	emax6.lmmi[0][7][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][7][0][emax6.lmmic].top = d070w[0];
	*(Uint*)&emax6.lmmi[0][8][0][emax6.lmmic] = 0x00001007|(1<<2);
	emax6.lmmi[0][8][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][8][0][emax6.lmmic].top = d080w[0];
	*(Uint*)&emax6.lmmi[0][9][0][emax6.lmmic] = 0x00001007|(1<<2);
	emax6.lmmi[0][9][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][9][0][emax6.lmmic].top = d090w[0];
	*(Uint*)&emax6.lmmi[0][10][0][emax6.lmmic] = 0x00001007|(1<<2);
	emax6.lmmi[0][10][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][10][0][emax6.lmmic].top = d100w[0];
	*(Uint*)&emax6.lmmi[0][11][0][emax6.lmmic] = 0x00001007|(1<<2);
	emax6.lmmi[0][11][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][11][0][emax6.lmmic].top = d110w[0];
	*(Uint*)&emax6.lmmi[0][12][0][emax6.lmmic] = 0x00001007|(1<<2);
	emax6.lmmi[0][12][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][12][0][emax6.lmmic].top = d120w[0];
	*(Uint*)&emax6.lmmi[0][13][0][emax6.lmmic] = 0x00001007|(1<<2);
	emax6.lmmi[0][13][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][13][0][emax6.lmmic].top = d130w[0];
	*(Uint*)&emax6.lmmi[0][14][0][emax6.lmmic] = 0x00001007|(1<<2);
	emax6.lmmi[0][14][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][14][0][emax6.lmmic].top = d140w[0];
	*(Uint*)&emax6.lmmi[0][15][0][emax6.lmmic] = 0x00001007|(1<<2);
	emax6.lmmi[0][15][0][emax6.lmmic].ofs = 0;
	emax6.lmmi[0][15][0][emax6.lmmic].top = d150w[0];
	emax6.lmmi_bitmap[0] = 0x000000000000fffeLL;
	emax6.lmmi_bitmap[1] = 0x0000000000000000LL;
	emax6.lmmi_bitmap[2] = 0x0000000000000001LL;
	emax6.lmmi_bitmap[3] = 0x0000000000000000LL;
	emax6_pre_with_drain_cache();
	get_nanosec(NANOS_ARM);
	if (emax6.last_conf == emax6_conf_inv_x2) {
	  emax6.status = STATUS_DRAIN;
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 1, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 2, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 3, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 4, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 5, 0);/*drain*/
	  ...
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 14, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 15, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 0, 2);/*drain*/
	}
	get_nanosec(NANOS_DRAIN);
	if (emax6.last_conf != emax6_conf_inv_x2) {
	  Dll *dst, *src;
	  int i,j;
	  emax6.status = STATUS_CONF;
	  emax6.last_conf = emax6_conf_inv_x2;
	  emax6.lastdist = 0;
	  dst = (Dll*)(((struct reg_ctrl*)emax6.reg_ctrl)->i[0].conf);
	  src = (Dll*)emax6_conf_inv_x2;
	  for (i=0; i<sizeof(conf)/sizeof(Dll); i++)
	    *dst++ = *src++;
	  for (i=0; i<64; i++) {
	    for (j=0; j<4; j++)
	...
	get_nanosec(NANOS_CONF);
	emax6.status = STATUS_REGV;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[0][0].br[0] = Ajk;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[0][0].br[1] = BR[2][3][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[1][0].br[0] = Ajk;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[1][0].br[1] = BR[3][3][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[2][0].br[0] = Ajk;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[2][0].br[1] = BR[4][3][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[3][0].br[0] = Ajk;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[3][0].br[1] = BR[5][3][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[4][0].br[0] = Ajk;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[4][0].br[1] = BR[6][3][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[5][0].br[0] = Ajk;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[5][0].br[1] = BR[7][3][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[6][0].br[0] = Ajk;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[6][0].br[1] = BR[8][3][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[7][0].br[0] = Ajk;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[7][0].br[1] = BR[9][3][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[8][0].br[0] = Ajk;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[8][0].br[1] = BR[10][3][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[9][0].br[0] = Ajk;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[9][0].br[1] = BR[11][3][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[10][0].br[0] = Ajk;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[10][0].br[1] = BR[12][3][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[11][0].br[0] = Ajk;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[11][0].br[1] = BR[13][3][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[12][0].br[0] = Ajk;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[12][0].br[1] = BR[14][3][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[13][0].br[0] = Ajk;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[13][0].br[1] = BR[15][3][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[14][0].br[0] = Ajk;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[14][0].br[1] = BR[16][3][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[63][0].br[0] = LOOP0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[63][0].br[1] = -1LL;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[63][0].br[2] = cofs;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[63][0].br[3] = 4LL;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[63][1].br[0] = b000;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[63][1].br[1] = Ajk;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[63][1].br[2] = BR[1][3][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[0][2].ea0b = (Ull)0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[0][2].ea0o = (Ull)d000[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[1][0].ea0b = (Ull)0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[1][0].ea0o = (Ull)d010[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[2][0].ea0b = (Ull)0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[2][0].ea0o = (Ull)d020[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[3][0].ea0b = (Ull)0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[3][0].ea0o = (Ull)d030[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[4][0].ea0b = (Ull)0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[4][0].ea0o = (Ull)d040[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[5][0].ea0b = (Ull)0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[5][0].ea0o = (Ull)d050[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[6][0].ea0b = (Ull)0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[6][0].ea0o = (Ull)d060[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[7][0].ea0b = (Ull)0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[7][0].ea0o = (Ull)d070[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[8][0].ea0b = (Ull)0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[8][0].ea0o = (Ull)d080[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[9][0].ea0b = (Ull)0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[9][0].ea0o = (Ull)d090[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[10][0].ea0b = (Ull)0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[10][0].ea0o = (Ull)d100[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[11][0].ea0b = (Ull)0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[11][0].ea0o = (Ull)d110[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[12][0].ea0b = (Ull)0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[12][0].ea0o = (Ull)d120[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[13][0].ea0b = (Ull)0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[13][0].ea0o = (Ull)d130[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[14][0].ea0b = (Ull)0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[14][0].ea0o = (Ull)d140[0];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[15][0].ea0b = (Ull)0;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[15][0].ea0o = (Ull)d150[0];
	get_nanosec(NANOS_REGV);
	emax6.status = STATUS_RANGE;
	{struct reg_ctrl *reg_ctrl = emax6.reg_ctrl;
	 Uint            lmmic     = emax6.lmmic;
	*(Ull*)&(reg_ctrl->i[0].addr[0][2].top) = ((Ull)(emax6.lmmi[0][0][2][lmmic].top+*((Ushort*)&emax6.lmmi[0][0][2][lmmic]+1)*sizeof(Uint)+(sizeof(Uint)-1))<<32) | (Ull)(Uint)emax6.lmmi[0][0][2][lmmic].top;
	...
	*(Ull*)&(reg_ctrl->i[0].addr[15][0].top) = ((Ull)(emax6.lmmi[0][15][0][lmmic].top+*((Ushort*)&emax6.lmmi[0][15][0][lmmic]+1)*sizeof(Uint)+(sizeof(Uint)-1))<<32) | (Ull)(Uint)emax6.lmmi[0][15][0][lmmic].top;
	...
	emax6_check_lmmi_and_dma(0, 2, emax6.lastdist, 0, 14, 0);/*load*/
	emax6_check_lmmi_and_dma(0, 2, emax6.lastdist, 0, 15, 0);/*load*/
	emax6_check_lmmi_and_dma(0, 2, emax6.lastdist, 0, 0, 2);/*load*/
	get_nanosec(NANOS_LOAD);
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].cmd = 3LL; // EXEC
	{struct reg_ctrl *reg_ctrl = emax6.reg_ctrl;
	 Uint            lmmic     = emax6.lmmic;
	}
	emax6.lmmd[0][2] = 0xff>>7;
	emax6.lmmd[1][0] = 0xff>>7;
	emax6.lmmd[2][0] = 0xff>>7;
	emax6.lmmd[3][0] = 0xff>>7;
	emax6.lmmd[4][0] = 0xff>>7;
	emax6.lmmd[5][0] = 0xff>>7;
	...
	while (((struct reg_ctrl*)emax6.reg_ctrl)->i[0].stat); //LMRING_BUSY|EXRING_BUSY
	get_nanosec(NANOS_EXEC);
asm volatile("b emax6_conf_end_inv_x2\n"
".align 5\n"
".global emax6_conf_inv_x2\n"
"emax6_conf_inv_x2:\n"
"	.word	0x00220005, 0x00000000\n"
"	.word	0xffff0000, 0x00000000\n"
"	.word	0x00000000, 0x00000000\n"
"	.word	0x00000000, 0x00000000\n"
"	.word	0x006240ad, 0x00000000\n"
"	.word	0xffff0000, 0x00000000\n"
"	.word	0x00000000, 0x00000000\n"
"	.word	0xffffffff, 0x00000000\n"
"	.word	0x60a28023, 0x00000000\n"
"	.word	0xffff0000, 0x00000e12\n"
"	.word	0xab000000, 0x00068060\n"
"	.word	0x00000000, 0x00000000\n"
"	.word	0x00000001, 0x00000000\n"
"	.word	0xffff0000, 0x00000000\n"
"	.word	0x00000000, 0x00000000\n"
"	.word	0x00000000, 0x00000000\n"
"	.word	0x10034023, 0x00000000\n"
"	.word	0xffff0000, 0x00000e12\n"
"	.word	0xab000000, 0x00068060\n"
"	.word	0x00000000, 0x00000000\n"
"	.word	0x00000001, 0x00000000\n"
...
"	.word	0x00000000, 0x00000000\n"
"	.word	0x00000000, 0x00000000\n"
".global emax6_conf_end_inv_x2\n"
"emax6_conf_end_inv_x2:\n"
);
#endif
#ifndef EMAXSC
	emax6_pre_with_drain_cache();
 ...
 Uint len = 256 -j-1;
   ...
   for (CHIP=0; CHIP<1; CHIP++) {
     for (h=0; h<16; h++) {
       for (k=256 -1; k>j; k--) {
  b[(i+CHIP*16 +h)*256 +j] -= A[j*256 +k]*x[(i+CHIP*16 +h)*256 +k];
       }
     }
   }
 }
 ...
 Uint *d080w[1],*d090w[1],*d100w[1],*d110w[1],*d120w[1],*d130w[1],*d140w[1],*d150w[1];
 for (CHIP=0; CHIP<1; CHIP++) {
   l000[CHIP] = (i+CHIP*16 + 0+0)*256 +j+1; l010[CHIP] = (i+CHIP*16 + 1+0)*256 +j+1; l020[CHIP] = (i+CHIP*16 + 2+0)*256 +j+1; l030[CHIP] = (i+CHIP*16 + 3+0)*256 +j+1;
   l040[CHIP] = (i+CHIP*16 + 4+0)*256 +j+1; l050[CHIP] = (i+CHIP*16 + 5+0)*256 +j+1; l060[CHIP] = (i+CHIP*16 + 6+0)*256 +j+1; l070[CHIP] = (i+CHIP*16 + 7+0)*256 +j+1;
   l080[CHIP] = (i+CHIP*16 + 8+0)*256 +j+1; l090[CHIP] = (i+CHIP*16 + 9+0)*256 +j+1; l100[CHIP] = (i+CHIP*16 +10+0)*256 +j+1; l110[CHIP] = (i+CHIP*16 +11+0)*256 +j+1;
   l120[CHIP] = (i+CHIP*16 +12+0)*256 +j+1; l130[CHIP] = (i+CHIP*16 +13+0)*256 +j+1; l140[CHIP] = (i+CHIP*16 +14+0)*256 +j+1; l150[CHIP] = (i+CHIP*16 +15+0)*256 +j+1;
...
SCM0[15].b[0][0] = (Ull)0;
SCM0[15].o[0][0] = (Ull)d150[0];
sc_param[0].LOOP0=LOOP0; sc_param[0].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[0], 0, emax6sc_pth_inv_x3_00, &sc_param[0]);
sc_param[1].LOOP0=LOOP0; sc_param[1].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[1], 0, emax6sc_pth_inv_x3_01, &sc_param[1]);
sc_param[2].LOOP0=LOOP0; sc_param[2].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[2], 0, emax6sc_pth_inv_x3_02, &sc_param[2]);
sc_param[3].LOOP0=LOOP0; sc_param[3].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[3], 0, emax6sc_pth_inv_x3_03, &sc_param[3]);
sc_param[4].LOOP0=LOOP0; sc_param[4].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[4], 0, emax6sc_pth_inv_x3_04, &sc_param[4]);
sc_param[5].LOOP0=LOOP0; sc_param[5].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[5], 0, emax6sc_pth_inv_x3_05, &sc_param[5]);
sc_param[6].LOOP0=LOOP0; sc_param[6].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[6], 0, emax6sc_pth_inv_x3_06, &sc_param[6]);
sc_param[7].LOOP0=LOOP0; sc_param[7].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[7], 0, emax6sc_pth_inv_x3_07, &sc_param[7]);
sc_param[8].LOOP0=LOOP0; sc_param[8].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[8], 0, emax6sc_pth_inv_x3_08, &sc_param[8]);
sc_param[9].LOOP0=LOOP0; sc_param[9].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[9], 0, emax6sc_pth_inv_x3_09, &sc_param[9]);
sc_param[10].LOOP0=LOOP0; sc_param[10].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[10], 0, emax6sc_pth_inv_x3_10, &sc_param[10]);
sc_param[11].LOOP0=LOOP0; sc_param[11].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[11], 0, emax6sc_pth_inv_x3_11, &sc_param[11]);
sc_param[12].LOOP0=LOOP0; sc_param[12].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[12], 0, emax6sc_pth_inv_x3_12, &sc_param[12]);
sc_param[13].LOOP0=LOOP0; sc_param[13].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[13], 0, emax6sc_pth_inv_x3_13, &sc_param[13]);
sc_param[14].LOOP0=LOOP0; sc_param[14].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[14], 0, emax6sc_pth_inv_x3_14, &sc_param[14]);
sc_param[15].LOOP0=LOOP0; sc_param[15].LOOP1=LOOP1; pthread_create((pthread_t*)&sc_pth[15], 0, emax6sc_pth_inv_x3_15, &sc_param[15]);
pthread_join(*(pthread_t*)&sc_pth[0], 0);
pthread_join(*(pthread_t*)&sc_pth[1], 0);
pthread_join(*(pthread_t*)&sc_pth[2], 0);
pthread_join(*(pthread_t*)&sc_pth[3], 0);
...
/* EMAXSC end */
	...
	*(Uint*)&emax6.lmmi[0][15][0][emax6.lmmic] = 0x00001007|(1<<2);
	...
	emax6_pre_with_drain_cache();
	get_nanosec(NANOS_ARM);
	if (emax6.last_conf == emax6_conf_inv_x3) {
	  emax6.status = STATUS_DRAIN;
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 1, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 2, 0);/*drain*/
	  emax6_check_lmmi_and_dma(0, 1, 0, 0, 3, 0);/*drain*/
	...
	if (emax6.last_conf != emax6_conf_inv_x3) {
	  ...
	  int i,j;
	  emax6.status = STATUS_CONF;
	  emax6.last_conf = emax6_conf_inv_x3;
	  emax6.lastdist = 0;
	  dst = (Dll*)(((struct reg_ctrl*)emax6.reg_ctrl)->i[0].conf);
	  src = (Dll*)emax6_conf_inv_x3;
	  for (i=0; i<sizeof(conf)/sizeof(Dll); i++)
	    *dst++ = *src++;
	  for (i=0; i<64; i++) {
	    for (j=0; j<4; j++)
	...
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[63][0].br[1] = -1LL;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[63][0].br[2] = cofs;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[63][0].br[3] = -(4);
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[63][1].br[0] = b000;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[63][1].br[1] = Ajk;
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].breg[63][1].br[2] = BR[1][3][1];
	((struct reg_ctrl*)emax6.reg_ctrl)->i[0].addr[0][2].ea0b = (Ull)0;
	...
	*(Ull*)&(reg_ctrl->i[0].addr[15][0].top) = ((Ull)(emax6.lmmi[0][15][0][lmmic].top+*((Ushort*)&emax6.lmmi[0][15][0][lmmic]+1)*sizeof(Uint)+(sizeof(Uint)-1))<<32) | (Ull)(Uint)emax6.lmmi[0][15][0][lmmic].top;
	...
	while (((struct reg_ctrl*)emax6.reg_ctrl)->i[0].stat); //LMRING_BUSY|EXRING_BUSY
	get_nanosec(NANOS_EXEC);
asm volatile("b emax6_conf_end_inv_x3\n"
".align 5\n"
".global emax6_conf_inv_x3\n"
"emax6_conf_inv_x3:\n"
"	.word	0x00220005, 0x00000000\n"
"	.word	0xffff0000, 0x00000000\n"
"	.word	0x00000000, 0x00000000\n"
...
"	.word	0x00000000, 0x00000000\n"
"	.word	0x00000000, 0x00000000\n"
".global emax6_conf_end_inv_x3\n"
"emax6_conf_end_inv_x3:\n"
);
#endif
#ifndef EMAXSC
	emax6_pre_with_drain_cache();
	...
	get_nanosec(NANOS_DRAIN);
      ...
      for (CHIP=0; CHIP<1; CHIP++) {
 for (h=0; h<16; h++) {
   inv1[j*256 +p[i+CHIP*16 +h]] = x[(i+CHIP*16 +h)*256 +j] = A[j*256 +j]*b[(i+CHIP*16 +h)*256 +j];
 }
      }
    }
  }
