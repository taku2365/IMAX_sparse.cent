...
/*3*/ for (CHIP=0; CHIP<NCHIP; CHIP++) {
  /*2*/ for (INIT1=1,LOOP1=RMGRP,rofs=0-M*4; LOOP1--; INIT1=0) {                             /* stage#0 *//* mapped to FOR() on BR[63][1][0] */
    /*1*/ for (INIT0=1,LOOP0=M-(i+1),cofs=0; LOOP0--; INIT0=0) { /* stage#0 *//* mapped to FOR() on BR[63][0][0] */
	    ...
	    mop(OP_LDUWR, 1, &BR[2][2][1], top,         cofs, MSK_W0, topw,       len, 0, 0, NULL, len);  /* A[p[i]*M+k]                       stage#2              |      */
	    mop(OP_LDUWR, 1, &BR[2][0][1], d00[CHIP],   oofs, MSK_W0, d00w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#2  +->         |      */
	    mop(OP_LDUWR, 1, &BR[2][1][1], d00[CHIP],   rofs, MSK_W0, d00w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#2  +->         |      */
	    ...
	    mop(OP_LDUWR, 1, &BR[3][2][1], top,         cofs, MSK_W0, topw,       len, 0, 0, NULL, len);  /* A[p[i]*M+k]                       stage#3              |      */
	    mop(OP_LDUWR, 1, &BR[3][0][1], d01[CHIP],   oofs, MSK_W0, d01w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#3  +->         |      */
	    mop(OP_LDUWR, 1, &BR[3][1][1], d01[CHIP],   rofs, MSK_W0, d01w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#3  +->         |      */
	    ...
	    mop(OP_LDUWR, 1, &BR[4][2][1], top,         cofs, MSK_W0, topw,       len, 0, 0, NULL, len);  /* A[p[i]*M+k] stage#1               stage#4              |      */
	    mop(OP_LDUWR, 1, &BR[4][0][1], d02[CHIP],   oofs, MSK_W0, d02w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#4  +->         |      */
	    mop(OP_LDUWR, 1, &BR[4][1][1], d02[CHIP],   rofs, MSK_W0, d02w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#4  +->         |      */
	    ...
	    mop(OP_LDUWR, 1, &BR[5][2][1], top,         cofs, MSK_W0, topw,       len, 0, 0, NULL, len);  /* A[p[i]*M+k] stage#1               stage#5              |      */
	    mop(OP_LDUWR, 1, &BR[5][0][1], d03[CHIP],   oofs, MSK_W0, d03w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#5  +->         |      */
	    mop(OP_LDUWR, 1, &BR[5][1][1], d03[CHIP],   rofs, MSK_W0, d03w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#5  +->         |      */
	    ...
	    mop(OP_LDUWR, 1, &BR[6][2][1], top,         cofs, MSK_W0, topw,       len, 0, 0, NULL, len);  /* A[p[i]*M+k]                       stage#6              |      */
	    mop(OP_LDUWR, 1, &BR[6][0][1], d04[CHIP],   oofs, MSK_W0, d04w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#6  +->         |      */
	    mop(OP_LDUWR, 1, &BR[6][1][1], d04[CHIP],   rofs, MSK_W0, d04w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#6  +->         |      */
	    ...
	    mop(OP_LDUWR, 1, &BR[7][2][1], top,         cofs, MSK_W0, topw,       len, 0, 0, NULL, len);  /* A[p[i]*M+k]                       stage#7              |      */
	    mop(OP_LDUWR, 1, &BR[7][0][1], d05[CHIP],   oofs, MSK_W0, d05w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#7  +->         |      */
	    mop(OP_LDUWR, 1, &BR[7][1][1], d05[CHIP],   rofs, MSK_W0, d05w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#7  +->         |      */
	    ...
	    mop(OP_LDUWR, 1, &BR[8][2][1], top,         cofs, MSK_W0, topw,       len, 0, 0, NULL, len);  /* A[p[i]*M+k] stage#1               stage#8              |      */
	    mop(OP_LDUWR, 1, &BR[8][0][1], d06[CHIP],   oofs, MSK_W0, d06w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#8  +->         |      */
	    mop(OP_LDUWR, 1, &BR[8][1][1], d06[CHIP],   rofs, MSK_W0, d06w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#8  +->         |      */
	    ...
	    mop(OP_LDUWR, 1, &BR[9][2][1], top,         cofs, MSK_W0, topw,       len, 0, 0, NULL, len);  /* A[p[i]*M+k] stage#1               stage#9              |      */
	    mop(OP_LDUWR, 1, &BR[9][0][1], d07[CHIP],   oofs, MSK_W0, d07w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#9  +->         |      */
	    mop(OP_LDUWR, 1, &BR[9][1][1], d07[CHIP],   rofs, MSK_W0, d07w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#9  +->         |      */
	    ...
	    mop(OP_LDUWR, 1, &BR[10][2][1],top,         cofs, MSK_W0, topw,       len, 0, 0, NULL, len);  /* A[p[i]*M+k]                       stage#10             |      */
	    mop(OP_LDUWR, 1, &BR[10][0][1],d08[CHIP],   oofs, MSK_W0, d08w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#10 +->         |      */
	    mop(OP_LDUWR, 1, &BR[10][1][1],d08[CHIP],   rofs, MSK_W0, d08w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#10 +->         |      */
	    ...
	    mop(OP_LDUWR, 1, &BR[11][2][1],top,         cofs, MSK_W0, topw,       len, 0, 0, NULL, len);  /* A[p[i]*M+k]                       stage#11             |      */
	    mop(OP_LDUWR, 1, &BR[11][0][1],d09[CHIP],   oofs, MSK_W0, d09w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#11 +->         |      */
	    mop(OP_LDUWR, 1, &BR[11][1][1],d09[CHIP],   rofs, MSK_W0, d09w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#11 +->         |      */
	    ...
	    mop(OP_LDUWR, 1, &BR[12][2][1],top,         cofs, MSK_W0, topw,       len, 0, 0, NULL, len);  /* A[p[i]*M+k] stage#1               stage#12             |      */
	    mop(OP_LDUWR, 1, &BR[12][0][1],d10[CHIP],   oofs, MSK_W0, d10w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#12 +->         |      */
	    mop(OP_LDUWR, 1, &BR[12][1][1],d10[CHIP],   rofs, MSK_W0, d10w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#12 +->         |      */
	    ...
	    mop(OP_LDUWR, 1, &BR[13][2][1],top,         cofs, MSK_W0, topw,       len, 0, 0, NULL, len);  /* A[p[i]*M+k] stage#1               stage#13             |      */
	    mop(OP_LDUWR, 1, &BR[13][0][1],d11[CHIP],   oofs, MSK_W0, d11w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#13 +->         |      */
	    mop(OP_LDUWR, 1, &BR[13][1][1],d11[CHIP],   rofs, MSK_W0, d11w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#13 +->         |      */
	    ...
	    mop(OP_LDUWR, 1, &BR[14][2][1],top,         cofs, MSK_W0, topw,       len, 0, 0, NULL, len);  /* A[p[i]*M+k] stage#1               stage#14             |      */
	    mop(OP_LDUWR, 1, &BR[14][0][1],d12[CHIP],   oofs, MSK_W0, d12w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#14 +->         |      */
	    mop(OP_LDUWR, 1, &BR[14][1][1],d12[CHIP],   rofs, MSK_W0, d12w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#14 +->         |      */
	    ...
	    mop(OP_LDUWR, 1, &BR[15][2][1],top,         cofs, MSK_W0, topw,       len, 0, 0, NULL, len);  /* A[p[i]*M+k] stage#1               stage#15             |      */
	    mop(OP_LDUWR, 1, &BR[15][0][1],d13[CHIP],   oofs, MSK_W0, d13w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#15 +->         |      */
	    mop(OP_LDUWR, 1, &BR[15][1][1],d13[CHIP],   rofs, MSK_W0, d13w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#15 +->         |      */
	    ...
	    mop(OP_LDUWR, 1, &BR[16][2][1],top,         cofs, MSK_W0, topw,       len, 0, 0, NULL, len);  /* A[p[i]*M+k] stage#1               stage#16             |      */
	    mop(OP_LDUWR, 1, &BR[16][0][1],d14[CHIP],   oofs, MSK_W0, d14w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#16 +->         |      */
	    mop(OP_LDUWR, 1, &BR[16][1][1],d14[CHIP],   rofs, MSK_W0, d14w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#16 +->         |      */
	    ...
	    mop(OP_LDUWR, 1, &BR[17][2][1],top,         cofs, MSK_W0, topw,       len, 0, 0, NULL, len);  /* A[p[i]*M+k] stage#1               stage#17             |      */
	    mop(OP_LDUWR, 1, &BR[17][0][1],d15[CHIP],   oofs, MSK_W0, d15w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#17 +->         |      */
	    mop(OP_LDUWR, 1, &BR[17][1][1],d15[CHIP],   rofs, MSK_W0, d15w[CHIP], len2,0, 1, NULL, len2); /* A[p[j+h*NCHIP+CHIP]*M+k]          stage#17 +->         |      */
...
/*2*/ for (CHIP=0; CHIP<NCHIP; CHIP++) {
  /*1*/ for (INIT0=1,LOOP0=jc,cofs=0-4; LOOP0--; INIT0=0) { /* stage#0 *//* mapped to FOR() on BR[63][0][0] */
	...
	/*for (h=0; h<H; h++) {*/ /* vertical (parallel) execution */
	  ...
	  /* b[59][j]-=A[p[j]][0:j-1] b[59][0:j-1]     b[58][*] b[57][*] b[56][*] *//* kループ後,j入れ換えにはA[p[j]][*]のbroadcastと，b[j]切替えのPIOで済む */
	  /* FOLDING時は,少なくとも第0列がFOLDINGであることが必要(conv-c2c仕様) */
	  mop(OP_LDUWR, 1, &Ajk,         top,        cofs, MSK_W0, topw,        len, 0, 0, NULL, len);  /* A[p[j]*M+k]                  *//* stage#1.0 inv_x1の同位置topと偶然一致するとrdy=1となりkick_dmaが出ない */
	  mop(OP_LDUWR, 1, &BR[1][3][1], t000[CHIP], cofs, MSK_W0, t000w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#1.3  +->xxx      LD     */
	  mop(OP_LDUWR, 1, &b000,        d000[CHIP], 0,    MSK_W0, d000w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#2.0  |   ■■■  |      */
...
#if (H>1)
	  mop(OP_LDUWR, 1, &BR[2][3][1], t010[CHIP], cofs, MSK_W0, t010w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#2.3  +->xxx      LD     */
	  mop(OP_LDUWR, 1, &b000,        d010[CHIP], 0,    MSK_W0, d010w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#3.0  |   ■■■  |      */
...
#if (H>2)
	  mop(OP_LDUWR, 1, &BR[3][3][1], t020[CHIP], cofs, MSK_W0, t020w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#3.3  +->xxx      LD     */
	  mop(OP_LDUWR, 1, &b000,        d020[CHIP], 0,    MSK_W0, d020w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#4.0  |   ■■■  |      */
...
#if (H>3)
	  mop(OP_LDUWR, 1, &BR[4][3][1], t030[CHIP], cofs, MSK_W0, t030w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#4.3  +->xxx      LD     */
	  mop(OP_LDUWR, 1, &b000,        d030[CHIP], 0,    MSK_W0, d030w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#5.0  |   ■■■  |      */
...
#if (H>4)
	  mop(OP_LDUWR, 1, &BR[5][3][1], t040[CHIP], cofs, MSK_W0, t040w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#5.3  +->xxx      LD     */
	  mop(OP_LDUWR, 1, &b000,        d040[CHIP], 0,    MSK_W0, d040w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#6.0  |   ■■■  |      */
...
#if (H>5)
	  mop(OP_LDUWR, 1, &BR[6][3][1], t050[CHIP], cofs, MSK_W0, t050w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#6.3  +->xxx      LD     */
	  mop(OP_LDUWR, 1, &b000,        d050[CHIP], 0,    MSK_W0, d050w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#7.0  |   ■■■  |      */
...
#if (H>6)
	  mop(OP_LDUWR, 1, &BR[7][3][1], t060[CHIP], cofs, MSK_W0, t060w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#7.3  +->xxx      LD     */
	  mop(OP_LDUWR, 1, &b000,        d060[CHIP], 0,    MSK_W0, d060w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#8.0  |   ■■■  |      */
...
#if (H>7)
	  mop(OP_LDUWR, 1, &BR[8][3][1], t070[CHIP], cofs, MSK_W0, t070w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#8.3  +->xxx      LD     */
	  mop(OP_LDUWR, 1, &b000,        d070[CHIP], 0,    MSK_W0, d070w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#9.0  |   ■■■  |      */
...
#if (H>8)
	  mop(OP_LDUWR, 1, &BR[9][3][1], t080[CHIP], cofs, MSK_W0, t080w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#9.3  +->xxx      LD     */
	  mop(OP_LDUWR, 1, &b000,        d080[CHIP], 0,    MSK_W0, d080w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#10.0 |   ■■■  |      */
...
#if (H>9)
	  mop(OP_LDUWR, 1, &BR[10][3][1],t090[CHIP], cofs, MSK_W0, t090w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#10.3 +->xxx      LD     */
	  mop(OP_LDUWR, 1, &b000,        d090[CHIP], 0,    MSK_W0, d090w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#11.0 |   ■■■  |      */
...
#if (H>10)
	  mop(OP_LDUWR, 1, &BR[11][3][1],t100[CHIP], cofs, MSK_W0, t100w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#11.3 +->xxx      LD     */
	  mop(OP_LDUWR, 1, &b000,        d100[CHIP], 0,    MSK_W0, d100w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#12.0 |   ■■■  |      */
...
#if (H>11)
	  mop(OP_LDUWR, 1, &BR[12][3][1],t110[CHIP], cofs, MSK_W0, t110w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#12.3 +->xxx      LD     */
	  mop(OP_LDUWR, 1, &b000,        d110[CHIP], 0,    MSK_W0, d110w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#13.0 |   ■■■  |      */
...
#if (H>12)
	  mop(OP_LDUWR, 1, &BR[13][3][1],t120[CHIP], cofs, MSK_W0, t120w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#13.3 +->xxx      LD     */
	  mop(OP_LDUWR, 1, &b000,        d120[CHIP], 0,    MSK_W0, d120w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#14.0 |   ■■■  |      */
...
#if (H>13)
	  mop(OP_LDUWR, 1, &BR[14][3][1],t130[CHIP], cofs, MSK_W0, t130w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#14.3 +->xxx      LD     */
	  mop(OP_LDUWR, 1, &b000,        d130[CHIP], 0,    MSK_W0, d130w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#15.0 |   ■■■  |      */
...
#if (H>14)
	  mop(OP_LDUWR, 1, &BR[15][3][1],t140[CHIP], cofs, MSK_W0, t140w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#15.3 +->xxx      LD     */
	  mop(OP_LDUWR, 1, &b000,        d140[CHIP], 0,    MSK_W0, d140w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#16.0 |   ■■■  |      */
...
#if (H>15)
	  mop(OP_LDUWR, 1, &BR[16][3][1],t150[CHIP], cofs, MSK_W0, t150w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#16.3 +->xxx      LD     */
	  mop(OP_LDUWR, 1, &b000,        d150[CHIP], 0,    MSK_W0, d150w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#17.0 |   ■■■  |      */
...
//EMAX5A begin inv_x3 mapdist=0
  /*2*/ for (CHIP=0; CHIP<NCHIP; CHIP++) {
    /*1*/ for (INIT0=1,LOOP0=jc,cofs=jc*4; LOOP0--; INIT0=0) { /* stage#0 *//* mapped to FOR() on BR[63][0][0] */
	    ...
	    /* b[59][j]-=A[p[j]][M-1:j+1] x[59][M-1:j+1] b[58][*] b[57][*] b[56][*] *//* kループ後,j入れ換えにはA[p[j]][*]のbroadcastと，b[j]切替えのPIOで済む */
	    mop(OP_LDUWR, 1, &Ajk,         top,          cofs, MSK_W0, topw,        len, 0, 0, NULL, len);  /* A[p[j]*M+k]                  *//* stage#1.0 inv_x1の同位置topと偶然一致するとrdy=1となりkick_dmaが出ない */
	    mop(OP_LDUWR, 1, &BR[1][3][1], t000[CHIP],   cofs, MSK_W0, t000w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#1.3  +->xxx      LD     *//*read-modify-write + exe-loop*/
	    mop(OP_LDUWR, 1, &b000,        d000[CHIP],   0,    MSK_W0, d000w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#2.0  |   ■■■  |      */
	    exe(OP_FMS,      &b000,        b000,         EXP_H3210, Ajk, EXP_H3210, BR[1][3][1], EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);        /* stage#2.0  +- xxx+ST   v      */
	    mop(OP_STWR,  1, &b000,        0,      d000[CHIP], MSK_D0, d000w[CHIP], 1,   0, 1, NULL, 1);                                      /* stage#2.0  +--------- xxx     */
#if (H>1)
	    mop(OP_LDUWR, 1, &BR[2][3][1], t010[CHIP],   cofs, MSK_W0, t010w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#2.3  +->xxx      LD     *//*read-modify-write + exe-loop*/
	    mop(OP_LDUWR, 1, &b000,        d010[CHIP],   0,    MSK_W0, d010w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#3.0  |   ■■■  |      */
	    exe(OP_FMS,      &b000,        b000,         EXP_H3210, Ajk, EXP_H3210, BR[2][3][1], EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);        /* stage#3.0  +- xxx+ST   v      */
	    mop(OP_STWR,  1, &b000,        0,      d010[CHIP], MSK_D0, d010w[CHIP], 1,   0, 1, NULL, 1);                                      /* stage#3.0  +--------- xxx     */
#if (H>2)
	    mop(OP_LDUWR, 1, &BR[3][3][1], t020[CHIP],   cofs, MSK_W0, t020w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#3.3  +->xxx      LD     *//*read-modify-write + exe-loop*/
	    mop(OP_LDUWR, 1, &b000,        d020[CHIP],   0,    MSK_W0, d020w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#4.0  |   ■■■  |      */
	    exe(OP_FMS,      &b000,        b000,         EXP_H3210, Ajk, EXP_H3210, BR[3][3][1], EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);        /* stage#4.0  +- xxx+ST   v      */
	    mop(OP_STWR,  1, &b000,        0,      d020[CHIP], MSK_D0, d020w[CHIP], 1,   0, 1, NULL, 1);                                      /* stage#4.0  +--------- xxx     */
#if (H>3)
	    mop(OP_LDUWR, 1, &BR[4][3][1], t030[CHIP],   cofs, MSK_W0, t030w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#4.3  +->xxx      LD     *//*read-modify-write + exe-loop*/
	    mop(OP_LDUWR, 1, &b000,        d030[CHIP],   0,    MSK_W0, d030w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#5.0  |   ■■■  |      */
	    exe(OP_FMS,      &b000,        b000,         EXP_H3210, Ajk, EXP_H3210, BR[4][3][1], EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);        /* stage#5.0  +- xxx+ST   v      */
	    mop(OP_STWR,  1, &b000,        0,      d030[CHIP], MSK_D0, d030w[CHIP], 1,   0, 1, NULL, 1);                                      /* stage#5.0  +--------- xxx     */
#if (H>4)
	    mop(OP_LDUWR, 1, &BR[5][3][1], t040[CHIP],   cofs, MSK_W0, t040w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#5.3  +->xxx      LD     *//*read-modify-write + exe-loop*/
	    mop(OP_LDUWR, 1, &b000,        d040[CHIP],   0,    MSK_W0, d040w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#6.0  |   ■■■  |      */
	    exe(OP_FMS,      &b000,        b000,         EXP_H3210, Ajk, EXP_H3210, BR[5][3][1], EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);        /* stage#6.0  +- xxx+ST   v      */
	    mop(OP_STWR,  1, &b000,        0,      d040[CHIP], MSK_D0, d040w[CHIP], 1,   0, 1, NULL, 1);                                      /* stage#6.0  +--------- xxx     */
#if (H>5)
	    mop(OP_LDUWR, 1, &BR[6][3][1], t050[CHIP],   cofs, MSK_W0, t050w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#6.3  +->xxx      LD     *//*read-modify-write + exe-loop*/
	    mop(OP_LDUWR, 1, &b000,        d050[CHIP],   0,    MSK_W0, d050w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#7.0  |   ■■■  |      */
	    exe(OP_FMS,      &b000,        b000,         EXP_H3210, Ajk, EXP_H3210, BR[6][3][1], EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);        /* stage#7.0  +- xxx+ST   v      */
	    mop(OP_STWR,  1, &b000,        0,      d050[CHIP], MSK_D0, d050w[CHIP], 1,   0, 1, NULL, 1);                                      /* stage#7.0  +--------- xxx     */
#if (H>6)
	    mop(OP_LDUWR, 1, &BR[7][3][1], t060[CHIP],   cofs, MSK_W0, t060w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#7.3  +->xxx      LD     *//*read-modify-write + exe-loop*/
	    mop(OP_LDUWR, 1, &b000,        d060[CHIP],   0,    MSK_W0, d060w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#8.0  |   ■■■  |      */
	    exe(OP_FMS,      &b000,        b000,         EXP_H3210, Ajk, EXP_H3210, BR[7][3][1], EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);        /* stage#8.0  +- xxx+ST   v      */
	    mop(OP_STWR,  1, &b000,        0,      d060[CHIP], MSK_D0, d060w[CHIP], 1,   0, 1, NULL, 1);                                      /* stage#8.0  +--------- xxx     */
#if (H>7)
	    mop(OP_LDUWR, 1, &BR[8][3][1], t070[CHIP],   cofs, MSK_W0, t070w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#8.3  +->xxx      LD     *//*read-modify-write + exe-loop*/
	    mop(OP_LDUWR, 1, &b000,        d070[CHIP],   0,    MSK_W0, d070w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#9.0  |   ■■■  |      */
	    exe(OP_FMS,      &b000,        b000,         EXP_H3210, Ajk, EXP_H3210, BR[8][3][1], EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);        /* stage#9.0  +- xxx+ST   v      */
	    mop(OP_STWR,  1, &b000,        0,      d070[CHIP], MSK_D0, d070w[CHIP], 1,   0, 1, NULL, 1);                                      /* stage#9.0  +--------- xxx     */
#if (H>8)
	    mop(OP_LDUWR, 1, &BR[9][3][1], t080[CHIP],   cofs, MSK_W0, t080w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#9.3  +->xxx      LD     *//*read-modify-write + exe-loop*/
	    mop(OP_LDUWR, 1, &b000,        d080[CHIP],   0,    MSK_W0, d080w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#10.0 |   ■■■  |      */
	    exe(OP_FMS,      &b000,        b000,         EXP_H3210, Ajk, EXP_H3210, BR[9][3][1], EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);        /* stage#10.0 +- xxx+ST   v      */
	    mop(OP_STWR,  1, &b000,        0,      d080[CHIP], MSK_D0, d080w[CHIP], 1,   0, 1, NULL, 1);                                      /* stage#10.0 +--------- xxx     */
#if (H>9)
	    mop(OP_LDUWR, 1, &BR[10][3][1],t090[CHIP],   cofs, MSK_W0, t090w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#10.3 +->xxx      LD     *//*read-modify-write + exe-loop*/
	    mop(OP_LDUWR, 1, &b000,        d090[CHIP],   0,    MSK_W0, d090w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#11.0 |   ■■■  |      */
	    exe(OP_FMS,      &b000,        b000,         EXP_H3210, Ajk, EXP_H3210, BR[10][3][1],EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);        /* stage#11.0 +- xxx+ST   v      */
	    mop(OP_STWR,  1, &b000,        0,      d090[CHIP], MSK_D0, d090w[CHIP], 1,   0, 1, NULL, 1);                                      /* stage#11.0 +--------- xxx     */
#if (H>10)
	    mop(OP_LDUWR, 1, &BR[11][3][1],t100[CHIP],   cofs, MSK_W0, t100w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#11.3 +->xxx      LD     *//*read-modify-write + exe-loop*/
	    mop(OP_LDUWR, 1, &b000,        d100[CHIP],   0,    MSK_W0, d100w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#12.0 |   ■■■  |      */
	    exe(OP_FMS,      &b000,        b000,         EXP_H3210, Ajk, EXP_H3210, BR[11][3][1],EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);        /* stage#12.0 +- xxx+ST   v      */
	    mop(OP_STWR,  1, &b000,        0,      d100[CHIP], MSK_D0, d100w[CHIP], 1,   0, 1, NULL, 1);                                      /* stage#12.0 +--------- xxx     */
#if (H>11)
	    mop(OP_LDUWR, 1, &BR[12][3][1],t110[CHIP],   cofs, MSK_W0, t110w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#12.3 +->xxx      LD     *//*read-modify-write + exe-loop*/
	    mop(OP_LDUWR, 1, &b000,        d110[CHIP],   0,    MSK_W0, d110w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#13.0 |   ■■■  |      */
	    exe(OP_FMS,      &b000,        b000,         EXP_H3210, Ajk, EXP_H3210, BR[12][3][1],EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);        /* stage#13.0 +- xxx+ST   v      */
	    mop(OP_STWR,  1, &b000,        0,      d110[CHIP], MSK_D0, d110w[CHIP], 1,   0, 1, NULL, 1);                                      /* stage#13.0 +--------- xxx     */
#if (H>12)
	    mop(OP_LDUWR, 1, &BR[13][3][1],t120[CHIP],   cofs, MSK_W0, t120w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#13.3 +->xxx      LD     *//*read-modify-write + exe-loop*/
	    mop(OP_LDUWR, 1, &b000,        d120[CHIP],   0,    MSK_W0, d120w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#14.0 |   ■■■  |      */
	    exe(OP_FMS,      &b000,        b000,         EXP_H3210, Ajk, EXP_H3210, BR[13][3][1],EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);        /* stage#14.0 +- xxx+ST   v      */
	    mop(OP_STWR,  1, &b000,        0,      d120[CHIP], MSK_D0, d120w[CHIP], 1,   0, 1, NULL, 1);                                      /* stage#14.0 +--------- xxx     */
#if (H>13)
	    mop(OP_LDUWR, 1, &BR[14][3][1],t130[CHIP],   cofs, MSK_W0, t130w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#14.3 +->xxx      LD     *//*read-modify-write + exe-loop*/
	    mop(OP_LDUWR, 1, &b000,        d130[CHIP],   0,    MSK_W0, d130w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#15.0 |   ■■■  |      */
	    exe(OP_FMS,      &b000,        b000,         EXP_H3210, Ajk, EXP_H3210, BR[14][3][1],EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);        /* stage#15.0 +- xxx+ST   v      */
	    mop(OP_STWR,  1, &b000,        0,      d130[CHIP], MSK_D0, d130w[CHIP], 1,   0, 1, NULL, 1);                                      /* stage#15.0 +--------- xxx     */
#if (H>14)
	    mop(OP_LDUWR, 1, &BR[15][3][1],t140[CHIP],   cofs, MSK_W0, t140w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#15.3 +->xxx      LD     *//*read-modify-write + exe-loop*/
	    mop(OP_LDUWR, 1, &b000,        d140[CHIP],   0,    MSK_W0, d140w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#16.0 |   ■■■  |      */
	    exe(OP_FMS,      &b000,        b000,         EXP_H3210, Ajk, EXP_H3210, BR[15][3][1],EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);        /* stage#16.0 +- xxx+ST   v      */
	    mop(OP_STWR,  1, &b000,        0,      d140[CHIP], MSK_D0, d140w[CHIP], 1,   0, 1, NULL, 1);                                      /* stage#16.0 +--------- xxx     */
#if (H>15)
	    mop(OP_LDUWR, 1, &BR[16][3][1],t150[CHIP],   cofs, MSK_W0, t150w[CHIP], len, 0, 1, NULL, len);  /* b[(i+CHIP*W*H+h*W+0)*M+k]    *//* stage#16.3 +->xxx      LD     *//*read-modify-write + exe-loop*/
	    mop(OP_LDUWR, 1, &b000,        d150[CHIP],   0,    MSK_W0, d150w[CHIP], 1,   0, 1, NULL, 1);    /* b[(i+CHIP*W*H+h*W+0)*M+j]    *//* stage#17.0 |   ■■■  |      */
	    exe(OP_FMS,      &b000,        b000,         EXP_H3210, Ajk, EXP_H3210, BR[16][3][1],EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);        /* stage#17.0 +- xxx+ST   v      */
	    mop(OP_STWR,  1, &b000,        0,      d150[CHIP], MSK_D0, d150w[CHIP], 1,   0, 1, NULL, 1);                                      /* stage#17.0 +--------- xxx     */
#endif
